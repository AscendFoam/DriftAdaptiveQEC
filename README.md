...
- `docs/`: 方案、阶段结论、恢复治理文件
  ...
  - `docs/new_tasks/T-RISK-20260728-04_resource_preflight_abort_full_v5_20260730_102415.md` / `docs/new_tasks/T-RISK-20260728-04_resource_preflight_abort_full_v5b_20260807_003952.md` / `docs/new_tasks/T-RISK-20260830-01_resource_wall_feasibility_repair.md` / `cnn_fpga/benchmark/phase9_powered_twin_resource_forensics.py`：前两个V5 resource被不同boot-session证明的外部主机重启中断并永久只读；V5c则是同一boot内的真实wall terminal。只读取证重放验证37,920点sample chain和8 receipts，得到four-worker total=`4,293,111.851 s`、artifact=`129,165,799,929 B`及ideal至少15 workers的wall下界；15-worker的RSS/CPU/scheduler尚未获证明或授权。当前T04为Blocked，继续执行outcome-blind、exact-output资源修复；保持518 cells/2,085,888 rows、seed、horizon、RESET estimand、3,043门、margin和14-day gate不变，完整独立投影未PASS前不得重复启动full run
