...
- `docs/`: 方案、阶段结论、恢复治理文件
  ...
  - `docs/t9_2_4_twin_qualification_preregistration.json` / `configs/phase9/t9_2_4_twin_qualification.json` / `docs/t9_2_4_formal_runner_amendment_seal.json` / amendment config：T9.2.4 在正式 A/B outcome 访问前冻结的 immutable parent 与 additive runner child seal。parent 保留六态×七动作、五共同初态、16 probes、四 fault family、14,976 backend-round/384 trajectories、四段互斥seed及18 tolerance；child 以独立 Hermite/Mehler logical bridge 修复 finite-cutoff `expm` 映射，并增加1,824个 cutoff-12 共同确认 round，总计16,800。冻结 runner、physics-free verifier、raw density/isometry NPZ、19类 estimand、2,000次 cluster max-T bootstrap、零 postselection及 `PASS/NO_GO/INCOMPLETE` fail-closed；30/30 gates/mutations、240项 focused+adjacent tests通过，`formal_result_accessed=false`，尚不代表 dual-backend agreement或任何性能/外部 claim
