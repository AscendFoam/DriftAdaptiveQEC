...
- `docs/`: 方案、阶段结论、恢复治理文件
  ...
  - `docs/new_tasks/T-RISK-20260728-04_resource_preflight_v5_preregistration.md` / `docs/new_tasks/T-RISK-20260728-04_resource_preflight_wall_fail_full_v5c_20260827_220849.md` / `cnn_fpga/benchmark/phase9_powered_twin_preflight.py` / preformal audit：T04 powered qualification 的 resource/preformal V5 证据链。V5c 首次完成8/8 full-denominator receipts、227,328 rows、15,360 RESET/sidecar、78 objects/10.86GB、37,920连续采样、四worker峰值RSS 5.39GB和no-copy inventory，但deterministic-LPT正式总wall约4,293,112.1s，超过14-day门3.549x，故终态`START -> FAIL_RESOURCE_PREFLIGHT(wall)`；没有formal outcome、resource PASS或V2 seal。future failure report已增加`completed_stage_evidence`以保留late-stage projection/decision，25项聚焦测试通过，V5c本身未回写。全部twin/LER/lifetime/physical/hardware/official/Puviani/SOTA/rank字段仍为null
  - `docs/new_tasks/T-RISK-20260728-04_resource_preflight_abort_full_v5_20260730_102415.md` / `docs/new_tasks/T-RISK-20260728-04_resource_preflight_abort_full_v5b_20260807_003952.md` / `docs/new_tasks/T-RISK-20260830-01_resource_wall_feasibility_repair.md` / `cnn_fpga/benchmark/phase9_powered_twin_resource_forensics.py`：前两个V5 resource被不同boot-session证明的外部主机重启中断并永久只读；V5c则是同一boot内的真实wall terminal。只读取证重放验证37,920点sample chain和8 receipts，得到four-worker total=`4,293,111.851 s`、artifact=`129,165,799,929 B`及ideal至少15 workers的wall下界。第一轮backend-B修复以解析structured tensor执行等价pure-loss/qutrit Kraus并缓存固定对象；dense reference max-abs `<=2e-16`、cutoff44 sense样本median `8.767x`、205/205 focused tests通过。该component结果不是完整518-cell投影；当前T04仍Blocked，保持518 cells/2,085,888 rows、seed、horizon、RESET estimand、3,043门、margin和14-day gate不变，完整独立投影未PASS前不得重复启动full run
