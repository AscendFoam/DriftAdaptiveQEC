...
文档总入口见 [`docs/README.md`](docs/README.md)。人类报告请从 [`docs/document_catalog/README.md`](docs/document_catalog/README.md) 按状态和 Phase 浏览，275 个顶层 JSON/CSV 请从 [`docs/evidence_catalog/README.md`](docs/evidence_catalog/README.md) 进入；不要从 `docs/` 顶层文件名猜测内容。

> **快照边界：** 当前 `mac_light` 工作树来自 2026-08-07 的跨机器开发快照，未携带部分 `runs/`、`artifacts/`、大体积 raw evidence、论文 PDF 和第三方二进制环境。代码与轻量文档可继续维护，但不能仅凭此工作树宣称完整证据可复现；原始打包说明已归档到 [`docs/legacy_context/travel_package_20260807/`](docs/legacy_context/travel_package_20260807/)。
...
- 自 `2026-07-14` 起，用户指定的“按新任务板顺序完整推进”工作流另以以下文件为当前执行状态源：
  ...
  - `docs/experiment_plan.md`：新任务序列的实验计划来源；第 14—20.9 节记录 v2—Phase 9 的低频修订；
...
- `physics/`: GKP 物理层与逻辑错误追踪
  - `physics/README.md`: 按基础/协议/多保真模型/验证研究/Phase 9 划分的阅读地图、复用原则和 byte-pinned 重构边界
  ...
  - `physics/control_imperfections.py` + `physics/_control_imperfections/validation.py`: AWG/DAC、pulse/latency/rotation 物理模型与独立验证/reporting 边界
  ...
  - `physics/cross_fidelity_validation.py` + `physics/_cross_fidelity/reporting.py`: Fock/effective/noise-transfer/direct-syndrome 四 lane 计算与独立检查/reporting 边界
...
- `cnn_fpga/`: 数据、模型、解码器、运行时、HIL、benchmark 主模块
  ...
  - `cnn_fpga/rtl/gkp_route_a_converged_production_top.sv` / `cnn_fpga/benchmark/converged_rtl_formal.py`: T6.25.2 唯一 production manager+commit admission+六周期 core+Route-A policy/LKG 顶层；compositional unbounded safety、20-cycle BMC、真实 core atomic refinement、14 reachable witnesses、21 independent formal mutants，板测/fastest/multimode RTL claim 保持关闭
  - `cnn_fpga/benchmark/converged_long_rtl_qualification.py` / `cnn_fpga/runtime/converged_production_reference.py`: T6.25.3 对上述 exact converged top 重建独立 golden；10×100,000-cycle CXXRTL 比较全部 148-byte 公开输出，验证六周期/II=1、full-image/atomic commit、fault/recovery/transport 与 21 类语义 mutation；仍仅为 two-state pre-board 证据
  - `cnn_fpga/benchmark/converged_hardware_lane_qualification.py` / `cnn_fpga/rtl/gkp_route_a_converged_synth_top.sv`: T6.25.4 对同一 exact qualified top 做 GW2AR seeds 1/7/19 synthesis/P&R；报告 Fmax/resource/critical-path/6-cycle clock model 与解析功耗敏感性，板测字段保持 null，并显式标记 observability fold 可能主导时序
  - `cnn_fpga/benchmark/phase6d_dual_lane_evidence_matrix.py`: T6.26.3 将 multimode opened-positive/strongest-baseline NO-GO、single-mode exact-top RTL GO 与 learning Dropped/absent 分成不可互相补门的 claim/figure panel；67-row Source Data 逐行绑定 report/raw/config/code/source/hash
  - `cnn_fpga/benchmark/phase6d_final_dual_lane_gate.py`: T6.26.4 从两条 primary lane 的直接证据独立重算四态终门；当前为 `GO_RTL_ONLY`，仅开放 exact single-mode pre-board deterministic/atomic/fail-closed RTL claim，multimode SOTA、板测、fastest 与 learning rescue 均保持关闭
  - `cnn_fpga/benchmark/phase6d_claim_figure_delta.py` / `docs/figures/make_t7_1_5_phase6d_delta.py`: T7.1.5 在不覆盖历史图包的前提下冻结 Figure5 multimode-only 与 Figure6 exact-single-mode-RTL-only delta；13 elements、10 claims、11 snapshots、58 artifacts、94-row Source Data 与22类mutation阻断旧图改色、跨lane总分和estimate→measured升级
  ...
  - `cnn_fpga/benchmark/secondary_evidence_integrity_gate.py`: T6.19.3 六 lane 只读 atlas 与终态完整性门；从 Source Data 重算 CI/threshold/LER/tail/bootstrap/timing/resource/count，生成逐 cell provenance、24 类逐门 mutation 和 Python SVG/PDF/TIFF/PNG figure bundle，并禁止 global/cross-lane/estimate-as-measured 升级
...
- `docs/`: 方案、阶段结论、恢复治理文件
  ...
  - `docs/phase6d_claim_figure_delta.md` / `docs/t7_1_5_phase6d_claim_figure_delta.json` / Source Data：Phase6D 两图、10条final claim和历史快照的机器合同；MM strongest-baseline NO-GO/context/unavailable 与 RTL deterministic/atomic/fail-closed/board-null 分图分caption
  - `docs/figures/t7_1_5_phase6d_delta/`：新 Figure5--6 的 Python-only editable SVG/PDF、300-dpi PNG、600-dpi LZW-TIFF及视觉QA manifest；输出 hash 与 T7.1.2--T7.1.4 全部历史图不相交
  ...
  - `docs/supplementary_evidence_contract.md` / `docs/t7_2_5_supplementary_evidence_contract.json` / Source Data：T7.2.5 的 46-row 八态 Supplementary 合同；24/24 gates/mutations 绑定公式、冻结参数、完整 baseline/statistics、失败域、双百万周期 RTL、工具链和 Phase 6C task-signature/source-locator/non-mixing 边界
  - `docs/paper_notes/Phase6D_Dual_Lane_GKP_manuscript.tex` / `output/pdf/t7_2_6_phase6d_dual_lane/Phase6D_Dual_Lane_GKP_manuscript.pdf`：T7.2.6 当前 canonical manuscript；12 页并列但不混排 multimode strongest-baseline NO-GO 与 exact-single-mode deterministic/atomic/fail-closed pre-board RTL，CNN/student 仅 Dropped ablation，旧 51 页稿保持历史快照
  - `docs/phase6d_manuscript_delta.md` / `docs/t7_2_6_phase6d_manuscript_delta.json` / Source Data / visual QA：T7.2.6 的 10-claim、57-row lossless 正文合同；27/27 gates/mutations、直接 exact-MLD/K-MWM 一手引用、TeX/BibTeX 零诊断与 12/12 页 hash-bound 视觉 QA
  - `docs/cnn_centric_reviewer_response.md` / `docs/t7_3_2_cnn_centric_reviewer_contract.json` / Source Data：T7.3.2 的 pre-emptive reviewer package；24-row evidence ledger、23/23 gates/mutations 同时披露 legacy CNN/teacher/student 正结果与 task-signature 不可迁移边界，当前返修信诚实标为 `draft_with_placeholders`
  - `docs/experiment_relevance_reviewer_response.md` / `docs/t7_3_3_experiment_relevance_reviewer_contract.json` / Source Data：T7.3.3 的七级实验相关性证据梯；24-row ledger、24/24 gates/mutations 分开 literature、official-code reproduction、project simulation、mock HIL、pre-board RTL、全null真板与Phase8 Todo，并禁止 AQEC/Sivak 等外部物理证据迁移
  - `docs/postselection_breakeven_reviewer_response.md` / `docs/t7_3_4_postselection_breakeven_reviewer_contract.json` / Source Data：T7.3.4 的全分母/post-selection成本/break-even taxonomy 审稿包；24-row ledger、24/24 gates/mutations 保留conditional正结果及unit-rejection反转，只允许300 us finite-cutoff operational boundary，其他gain/break-even均未建立
  - `docs/exact_oracle_map_reviewer_response.md` / `docs/t7_3_1_exact_oracle_map_reviewer_contract.json` / Source Data：T7.3.1 的 20-row 七态审稿问答合同；20/20 gates/mutations 区分 frozen-model exact MAP、truth-privileged oracle 和 channel/control optimum，并锁定负 static-to-oracle gap、static/Window/calibration 反例及 held-out promotion gate
  - `docs/paper_notes/Contract_Centric_Regime_Aware_GKP_note_draft.tex`: 截止 T7.3.1 的 51 页 restricted pre-board Route-A 正式论文式 note；Introduction/Related Work、Methods、Results、8段Discussion/Conclusion、exact/oracle MAP 审稿回答和九段 Supplement 均按 contract/task-signature/evidence-state 边界组织，正式 Fig.1--4、Supplement S1--S5、N/A/null/failed 与 long-RTL denominator 不隐藏负结果、V5 stop、物理缺口或 board-null
  - `docs/paper_notes/CNN_FPGA_GKP_theory_note_draft.tex`: 截止 T7.2.5 的历史 companion note；保留原 CNN/affine/teacher--student/FPGA 主体和两张 PPT 图，只增加正式论文结构、成本、外部效度、Supplementary 审计与六级物理接入覆盖层，当前独立编译 33 页且不得覆盖 static/Window、V5 stop 或 board-null
  ...
  - `docs/t9_1_3_puviani_paper_constrained.json` / agent registry / selection、training、six-state raw ledgers / `docs/t9_1_3_post_outcome_governance_addendum.json` / Source Data：T9.1.3 的 paper-constrained MF/NMF artifact lane；20 paired roots、40 agents、40,000 training rows、1,008 six-state trajectories/20,160 events、52/52 parent gates、81/81 parent mutations与独立 live validation通过。addendum v2 以16/16 gates、14/14 dual-branch mutations和单向 semantic seal 承载 D43--D46；该结果仅为 `QUALIFIED_PAPER_CONSTRAINED_BASELINE`，不得追溯改写 parent；`official_exact`、`puviani_surpass`、`paper_scale_lifetime` 三个实际 claim slot 保持 typed null，physical/SOTA claim 不开放且 matched/SOTA eligibility=false
  - `docs/phase9_baseline_search_power_registry.md` / `docs/t9_1_4_baseline_search_power_registry.json` / 两份 CSV / `configs/phase9/t9_1_4_baseline_registry.json`：T9.1.4 的 pre-outcome baseline/search/power 冻结；31 个方法含 18 个 mandatory matched baseline，另分 capacity ceiling、privileged oracle/teacher 与 protocol anchor。primary 输入为共同 bit-accurate frontend 的定点 LLR，action-conditioned pairing 只共享初态与外生创新；round-LER 功效为 required 806、planned 808 clusters/backend，lifetime 功效保持 typed null。文献账本把 23 个 bibliographic hits 去重为 12 个 work family，4 条 repository/local/dataset evidence 单独附着；36/36 gates/mutations 通过，但所有 current rank、external-SOTA、official/Puviani、physical lifetime 仍为 null
  - `docs/phase9_scoped_claim_amendment.md` / `docs/t9_1_5_scoped_claim_amendment.json` / Source Data / `configs/phase9/t9_1_5_scoped_claim_amendment.json` / `t9_1_5_release_pin.json`：T9.1.5 的 parent-bound scoped-claim v2 child seal；byte+semantic 绑定 immutable T9.1.1 与 T9.1.4，不回写 parent，并以 report 外部 release pin 固定 canonical analysis/config/implementation/output。29 条 lane-qualified legacy migration 只允许 legacy GO 成为 narrower registered/project-native/integrated candidate 且不自动迁移；9 个 typed-null state 分离 registered/external LER、project/external/physical lifetime、official Puviani exact/surpass 与 recorded/live/integrated/external-speed HIL。strict SHA/count/threshold types、17 条 forbidden transfer、exact-closure/sequence/previous-entry hash chain 与 externally pinned snapshot anchor 的可执行 revocation、27 条 value/verdict 全-null predicate fixtures、4 条 propagation fixtures、127-row full-analysis lossless Source Data 和 36/36 gates/mutations 共同阻止 fixture/任意 Mapping→GO、registered→external、simulation→physical、recorded/preboard→live measured、六周期→端到端、重哈希历史、NaN/假hash与 null→false/zero；当前所有真实 claim/performance/rank 仍为 null
  - `docs/phase9_causal_twin_contract.md` / `docs/t9_2_1_causal_twin_contract.json` / totality manifest / Source Data / `configs/phase9/t9_2_1_causal_twin_contract.json` / `t9_2_1_release_pin.json`：T9.2.1 的 pre-backend 因果接口与有限总递推合同；冻结五个 namespace、observed+memory 唯一 deployable API、24-bit K、单参数 `F(K)`、1,024-cell N、131,072-cell T、12-signature/196,608-cell lossless composition quotient、80-bit semantic+CRC action、previous-K recurrence receipt、14级 fail-closed priority、完整 trusted-package nomination 与16个 runtime-witnessed conservative probes。40/40 gates、40/40 mutations和62项 focused tests通过；该 PASS 不资格化 physics backend、frontend、codebook、当前 RTL adapter、LER/lifetime、硬件、official/Puviani、external SOTA 或 rank
  - `docs/phase9_backend_a_qualification.md` / `docs/t9_2_2_backend_a_qualification.json` / Source Data / `configs/phase9/t9_2_2_backend_a.json` / release pin：T9.2.2 的 joint finite-Fock oscillator × `g/e/f` qutrit backend A；恢复/Ramsey Hamiltonian、GKSL、continuous-IQ Kraus backaction、reset failure/`f` persistence、action-conditioned leakage/drift 与 evaluator-only 六态 projection 均实际执行。20/20 gates、20/20 mutations、27/27 core checks和63项 focused tests通过；这是 synthetic backend-A-only 资格，backend B/dual agreement、frontend、codebook、LER/lifetime、physical/HIL、official/Puviani、external SOTA 与 rank 仍为 null
  - `docs/phase9_backend_b_qualification.md` / `docs/t9_2_3_backend_b_qualification.json` / Source Data / `configs/phase9/t9_2_3_backend_b.json` / release pin：T9.2.3 的独立 backend B；不导入 backend A 或既有 Fock/SBS/GKP kernel，改用 dense midpoint Strang、解析 Kraus/Schur、BLAKE2b+Python Random+手写 Box–Muller、独立 Gaussian IQ likelihood/Kraus 与 squeezed-comb evaluator。pure-loss/qutrit-relaxation 闭式对拍、Choi/full physicality、真实 syndrome backaction、reset/f persistence、action/leakage/drift、seed 与 split/cutoff 收敛通过；22/22 gates、22/22 mutations、28/28 checks和52项 focused tests通过。只资格化 synthetic backend-B implementation；A/B agreement、性能、physical/HIL、official/Puviani、external SOTA 与 rank 仍为 null
  - `docs/t9_2_4_twin_qualification_preregistration.json` / child seal / formal report / Source Data / raw NPZ / release pin / post-outcome audit：T9.2.4 immutable parent+additive child下的唯一正式A/B资格。16,800/16,800 backend-round、0 exception、全conservation；独立verifier analysis=`d3f05e69...` 在1,042门中得到825 pass/217 fail，正式 `NO_GO_TWIN_QUALIFICATION`，主要失败为各66个IQ mean/KS/log-evidence，最差bound=`6.135524>0.3`。post-audit analysis=`b9b0de85...` 以25/25 gates、24/24 mutations独立确认ledger/NPZ/hash/最差点/零postselection/下游传播。该NO-GO阻塞surrogate、synthetic-IQ frontend资格、codebook与LER/lifetime formal，只释放T9.2.6 platform interface freeze；所有LER/lifetime/physical/hardware/official/Puviani/SOTA/rank字段保持null
  - `docs/phase9_raw_iq_platform_envelope.md` / `docs/t9_2_6_raw_iq_platform_envelope.json` / Source Data / `configs/phase9/t9_2_6_raw_iq_platform_envelope.json` / release pin：T9.2.6 的 raw-IQ/AXIS/fixed-point/package/platform design envelope，analysis=`b10a035f...`。冻结125/250 MS/s×512 ns、I16/Q16、35/48-bit complex MAC、66/67-bit calibration、32/128-bit AXIS、16类fault、192-cycle deadline、poison-once quarantine、atomic reset flush、complete A/B package/CRC/version/SHA/LKG、trusted receipt=null/current activation=0、ZCU111/ZCU216候选交集和四个不可迁移 latency boundary。36/36 gates/mutations、85 focused tests通过；frontend performance、recorded/live IQ、综合/P&R/真板、official/Puviani/SOTA/rank等15字段仍为null，T9.2.7保持Blocked
  - `docs/new_tasks/T-RISK-20260726-01_fresh_twin_iq_likelihood_repair.md` / `docs/t_risk_20260726_01_fresh_*` / `configs/phase9/t_risk_20260726_01_*`：不回写 T9.2.4 的 fresh twin 修复与正式资格。第三解析锚、predictive IQ/common-heldout、RB reset、cellwise TOST/IUT、768/256 power、592 chunks/528,384 rows/0 exception 和 physics-free verifier 均完成；正式 analysis=`e1539735...` 为1,562/1,589 pass、27 fail，verdict=`NO_GO_FRESH_TWIN_QUALIFICATION`。post-audit=`d065f2eb...`，40/40 gates、69/69 mutations；12→16及分态16→20 fault density未收敛，density UCB/六态聚合需新任务修复。六个downstream继续Blocked，15个performance/physical/hardware/official/Puviani/SOTA/rank字段全null
  - `docs/new_tasks/T-RISK-20260727-01_high_cutoff_state_design_pilot.md` / `docs/t_risk_20260727_01_high_cutoff_design_pilot_fresh3_manifest.json` / diagnostic / Source Data / completion：不回写两个既有 NO-GO 的高 cutoff 六态分层 design pilot。immutable fresh3 完成32 cells、64 chunks、27,648 unique rows、2,304终态density、0 exception/0 conservation failure；coverage-calibrated V2b diagnostic 重算3,900 gates，得到 `EXPLORATORY_RISK_SIGNAL`（analysis=`c4ddfba3...`）。24→28 density max=`0.091343>0.075`且cutoff28有6个绝对tail trigger，故只触发新的cutoff32/36修复任务；这不是 twin qualification PASS，六个downstream继续Blocked，所有performance/physical/hardware/official/Puviani/SOTA/rank字段保持null
  - `docs/new_tasks/T-RISK-20260728-02_scalar_uq_three_split_calibration.md` / `docs/t_risk_20260728_02_scalar_uq_calibration.json` / verification / 三份 LFS Source Data：原 factor=1/288-trial scalar UQ NO-GO 后的 fresh 双 selection-fold + untouched confirmation。V1 seed生日碰撞在receipt前fail-closed并永久保留；V2使用可证明单射的cell-major fresh seed，完成1,179,648 rows。最小factor仍为1.0，三个split最差coverage rate=`0.94092--0.94482`、simultaneous Wilson LCB=`0.91838--0.92286`，四组power IUT全PASS；独立verifier逐行重算通过。该结果只资格化后续tail UQ，不是twin/LER/lifetime/hardware/official/Puviani/SOTA证据
  - `docs/new_tasks/T-RISK-20260728-01_cutoff32_36_design_no_go.md` / `docs/t_risk_20260728_01_cutoff32_36_design_*` / V2 verification / NO-GO diagnosis：cutoff32/36 fresh2完成22 cells、14,256新raw rows；合并cutoff28后独立重算30 receipts、21,168 rows、2,160 densities和1,454 gates，得到1,393 pass/61 fail、verdict=`VERIFIED_NO_GO_HIGH_CUTOFF_DESIGN`。56项失败只在28→32，32→36的596门全过，但cutoff36仍有2个state-conditioned commutator-tail失败；shared RESET另有3项59/72对54/72独立branch抽样伪差异。powered formal未释放，下一任务只允许一次性fresh 36/40/44与Rao–Blackwellized expected RESET；全部performance/physical/hardware/official/Puviani/SOTA/rank字段保持null
  - `docs/new_tasks/T-RISK-20260728-03_cutoff36_44_bounded_repair.md` / `docs/t_risk_20260728_03_cutoff36_44_repair_fresh1_*` / `runs/t_risk_20260728_03_cutoff36_44_repair_fresh1/`：一次性bounded 36/40/44 design repair；30/30 cells、21,168 raw rows、2,160 primary densities、1,296 RESET sidecar densities、0 exception。Rao–Blackwell expected RESET逐branch可复算，diagnostic与independent verifier均为1,454/1,454，max ratio=`0.472112`、verification=`31ffddde...`；完整约206 MB run按manifest逐字节归档，NPZ走LFS。该PASS只授权另行预注册fresh powered formal，不是twin/LER/lifetime/physical/hardware/official/Puviani/SOTA证据；随后由T05/T06独立完成正式count功效设计
  - `docs/new_tasks/T-RISK-20260728-05_highdim_joint_maxt_no_go.md` / `docs/t_risk_20260728_05_highdim_joint_maxt_*` / full gate blueprint：T04前的高维UQ与完整门族统计预检。两个实现独立展开3,043门；6,144个d120/d132真实PSD paired-density trial的coverage全为1，但384/state下heteroskedastic 0.05 local两维均0/256。完整3,037随机门joint-maxT critical=`4.11475`，旧384/768计数的null global power=`0.433`、LCB=`0.4177`。physics-free verifier重建6,144 rows、3,043 gates、199 replicates和40 power cases，20/20 gates、最大差`2.50e-16`，正式确认statistical NO-GO；当时T04未释放，后续T06以有限count selection+disjoint confirmation修复设计功效，全部性能/physical/hardware/official/Puviani/SOTA claim仍null
  - `docs/new_tasks/T-RISK-20260728-06_count_selection_confirmation.md` / `docs/new_tasks/T-RISK-20260730-01_effect_dispatch_repair.md` / T06 report、Source Data、selected blueprint、independent verification与三条失败归档：T05 statistical NO-GO后的有限count selection + disjoint confirmation。四候选完整评估后选择最小passing scale=`2.0`，即state/round/fault=`768/1536/4608`；8+24 chunks、7,168 density trials、3,043门。V1 exact-float dispatch、多线程maxT序列化与verifier-input三类实现失败均fail-closed并永久归档；最终只改变六个canonical 0.05 summary的两个布尔字段，32 raw chunks和blueprint逐字节不变。physics-free verifier重算1,024+6,144 density、796+199 maxT replicates、160+40 power cases和4 linked blueprints，21/21、max delta=`1.94e-16`、analysis=`5a49967f...`；78 focused tests和144 live checks通过。只释放T-RISK-20260728-04 preregistration，twin/LER/lifetime/physical/hardware/official/Puviani/SOTA仍为null
  - `docs/new_tasks/T-RISK-20260728-04_resource_preflight_v5_preregistration.md` / `docs/new_tasks/T-RISK-20260728-04_resource_preflight_abort_full_20260730_080203.md` / `cnn_fpga/benchmark/phase9_powered_twin_preflight.py` / `cnn_fpga/benchmark/phase9_powered_twin_preformal_audit.py`：T04 powered qualification 的 resource/preformal V5 加固入口。旧V4在发现formal-LPT峰值、外部wall/PID、逐row/NPY、inventory/mapping-anchor和owner/attempt/seal消费链缺口后受控`START -> FAIL`并永久只读保留；无OOM、timeout或科学NO-GO证据。V5冻结8个full-denominator receipts、227,328行全读、逐cell artifact/transient、deterministic LPT、V2 preformal seal与downgrade拒绝；fresh V5 resource/正式资格未PASS前，全部twin/LER/lifetime/physical/hardware/official/Puviani/SOTA/rank字段仍为null
  ...
  - `docs/reports/phase6/multimode_causal_headroom.md` / `docs/t6_20_4_multimode_causal_headroom.json` / Source Data：Phase 6D train-only causal-headroom 与五段 regret；79,872 轮、official BSV/T-join/alias/causality/indirect-privilege audit 全过，trusted-bank risk action 把未保护 PP 的 49.6% 退化拉回 static exact，但 relative headroom 为 `0% [0%,0%]`，因此 direct NO-GO，pilot/formal 未访问
  ...
  - `docs/single_mode_rtl_boundary_audit.md` / `docs/t6_25_1_single_mode_rtl_boundary_audit.json` / Source Data：T6.25.1 的 5-top×11-capability live-source/实例化边界审计；确认 production management、policy long-run 与旧 P&R 尚未汇聚到同一 top，冻结 converged production top 的 property→百万周期→三种子 P&R 强制顺序
  - `docs/converged_rtl_formal.md` / `docs/t6_25_2_converged_rtl_formal.json` / Source Data：T6.25.2 的 unique converged top、分解无界安全证明、BMC/witness/mutation closure 与形式检查驱动的 duplicate-commit/host-CAS 修复；后续 long-run/P&R 必须绑定同一源码
  ...
  - `docs/reports/phase4/experimental_history_input.md` / `docs/t4_1_2_experimental_history_validation.json` / `.csv`: 256×53 observed-only schema、真实 producer 对齐、17-gate leakage/fault/saturation audit 与 16,384-row Source Data
  - `docs/reports/phase4/hybrid_state_output.md` / `docs/t4_1_3_hybrid_state_output_validation.json` / `.csv`: future hybrid state、T4.1.1 HMM bridge、block-bootstrap uncertainty、58 次 atomic commit 与 456-row Source Data
  - `docs/reports/phase4/hybrid_multiobjective_calibration.md` / `docs/t4_1_4_hybrid_multiobjective_validation.json` / `.csv`: 六项 loss、3/2/3 strict split、proper-score calibration、448-row future targets 与 fallback 无选择性的负结果
  - `docs/reports/phase4/offline_teacher_student_distillation.md` / `docs/t4_1_5_teacher_student_validation.json` / `.csv` / `docs/t4_1_5_distilled_student_checkpoint.json`: 5-agent frozen offline teacher、75-trainable/105-scalar recurrence student、15,360-row strict-split Source Data 与 physical-gain 未验证边界
  - `docs/reports/phase4/parametric_map_lut.md` / `docs/t4_2_1_parametric_map_lut_validation.json` / `.csv` / `docs/t4_2_1_parametric_map_lut_bank_images.json`: active K/b 反解、X/Z integer LLR ROM、16,384-row exhaustive grid、5-cycle/II=1 software pipeline 与 non-hardware 边界
  - `docs/reports/phase4/experimental_event_fsm.md` / `docs/t4_2_2_experimental_event_fsm_validation.json` / `.csv`: 六态 observed-event FSM、饱和 run counters、Pauli/phase-frame、1,024-row fault/transition replay 与 6-cycle/II=1 software contract
  - `docs/reports/phase4/conservative_fallback_health.md` / `docs/t4_2_3_conservative_fallback_validation.json` / `.csv`: 14-bit health/integrity registry、trusted image/version、frame-hold/reset fallback、4,096-row fault replay 与 non-hardware 边界
  - `docs/reports/phase4/fast_path_fixed_point.md` / `docs/t4_2_4_fast_path_fixed_point_validation.json` / `docs/t4_2_4_fast_path_fixed_point_ler.csv` / `docs/t4_2_4_fast_path_fixed_point_exhaustive_codes.csv`: MAP→health→event→frame 全 word contract、87,040-code audit 与 128-cluster paired LER impact
  - `docs/reports/phase4/three_timescale_cadence.md` / `docs/t4_3_1_three_timescale_cadence_validation.json` / `docs/t4_3_1_adaptation_lag_phase_sweep.csv` / `docs/t4_3_1_cadence_execution_trace.csv`: fast/event/window/slow/commit/recalibration cadence、4000-phase 双口径 adaptation lag 与真实 scheduler/T4.2 trace
  ...
  - `docs/reports/phase4/bounded_residual_rnn_teacher.md` / `docs/t4_4_1_bounded_residual_rnn_teacher_validation.json` / `.pt` / `.csv`: 3-restart fresh GRU teacher、15-output hard action bounds、1,074-row strict-split Source Data、checkpoint non-reuse 与 cap-hit/非全局收敛边界
  - `docs/reports/phase4/teacher_hidden_control_analysis.md` / `docs/t4_4_2_teacher_hidden_control_analysis.json` / `.csv`: 128-half-cycle hidden/control、strict-split p(g) belief probe、PCA/指数/有效记忆、2,089-row Source Data 与 leakage 非原生边界
  - `docs/reports/phase4/low_dimensional_student_distillation.md` / `docs/t4_4_3_low_dimensional_student_validation.json` / candidate `.pt` / student `.json` / `.csv`: 4-state/95-scalar selected recurrence、held-out imitation error、58,356-row Source Data 与 gain-retention 未验证边界
  ...
  - `docs/reports/phase6/route_a_posterior_calibration.md` / `docs/t6_6_3_route_a_posterior_threshold_lock.json` / Source Data: T6.6.3 全量 calibration/pilot、V4 common tuple/router/EWMA baseline lock、完整 pilot selector；V2/V3 NO-GO、fallback/lag 与 formal 未访问边界完整披露
  ...
  - `docs/secondary_comparison_atlas.md` / `docs/t6_19_3_secondary_evidence_integrity.json` / Source Data / `docs/figures/t6_19_3_secondary_comparison_atlas.*`: T6.19.3 206-cell六lane非主排名atlas；含18个外部FPGA实现×9个逐指标literature/null单元、raw统计重算、24/24 integrity gates/mutations和editable-text/600-dpi figure bundle；`PASS_AUX_COMPARISON_INTEGRITY`不改变Phase6B NO-GO或板测Blocked状态
  ...
  - `docs/reports/phase4/teacher_student_gain_retention.md` / `docs/t4_4_4_teacher_student_gain_retention.json` / `.csv`: cutoff12/16 paired retention point/CI、MF 排名反转、g/e/leakage burden、解析成本与 short-horizon oracle 边界
  - `docs/reports/phase4/teacher_student_branch_freeze.md` / `docs/t4_4_5_teacher_student_branch_freeze.json` / `.csv`: 8-predicate fail-closed branch freeze、112-row parent/claim/revocation ledger 与 armed MAP-LUT fallback
