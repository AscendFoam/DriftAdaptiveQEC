"""Outcome-free cutoff-32/36 density-UCB coverage and power preflight.

This transaction preserves the frozen factor=1.0 and B=199, and tests them on
joint-density dimensions 84/96/108 before any cutoff-32/36 scientific outcome
is read.  It cannot choose a new factor, sample count, margin, or paper claim.
"""
...
CONFIG_PATH = "configs/phase9/t_risk_20260728_01_density_uq_preflight.json"
CONFIG_SCHEMA = "PHASE9-CUTOFF32-36-DENSITY-UQ-PREFLIGHT-CONFIG-V1"
CHUNK_SCHEMA = "PHASE9-CUTOFF32-36-DENSITY-UQ-PREFLIGHT-CHUNK-V1"
REPORT_SCHEMA = "PHASE9-CUTOFF32-36-DENSITY-UQ-PREFLIGHT-V1"
LOCK_SCHEMA = "PHASE9-CUTOFF32-36-DENSITY-UQ-PREFLIGHT-OWNER-LOCK-V1"
RUN_IDENTITY_SCHEMA = "PHASE9-CUTOFF32-36-DENSITY-UQ-PREFLIGHT-RUN-IDENTITY-V1"
...
PASS_VERDICT = "PASS_CUTOFF32_36_DENSITY_UQ_PREFLIGHT"
NO_GO_VERDICT = "INCOMPLETE_CUTOFF32_36_DENSITY_UQ_PREFLIGHT"
CLAIM_BOUNDARY = {
...
def load_config(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        or config.get("dimensions") != [84, 96, 108]
    ...
    if (
        ...
        or int(split.get("trial_seed_base", 0)) != 1520000
        or int(split.get("multiplier_seed_base", 0)) != 1530000
    ):
    ...
    if (
        ...
        or int(preflight.get("trial_seed_base", 0)) != 1540000
        or int(preflight.get("multiplier_seed_base", 0)) != 1550000
        or float(preflight.get("eta_safety_factor", 0.0)) != 2.0
        or int(preflight.get("maximum_estimated_wall_seconds", 0)) != 14400
        or int(preflight.get("maximum_estimated_worker_pool_rss_bytes", 0))
        != 4294967296
        or int(preflight.get("maximum_estimated_total_task_rss_bytes", 0))
        != 5368709120
    ...
    if (
        ...
        != (
            ...
            "across all 72 coverage and 24 power intervals"
        )
    ):
