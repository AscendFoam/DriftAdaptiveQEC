"""Independent point-screen diagnostic for the cutoff-32/36 design extension.
...
SCHEMA = "PHASE9-CUTOFF32-36-DESIGN-DIAGNOSTIC-V1"
COMPLETION_SCHEMA = "PHASE9-CUTOFF32-36-DESIGN-DIAGNOSTIC-COMPLETION-V1"
LOCK_SCHEMA = "PHASE9-CUTOFF32-36-DESIGN-DIAGNOSTIC-LOCK-V1"
...
def _physicality(stack: np.ndarray, cutoff: int) -> None:
    ...
    if (
        float(np.max(np.abs(np.trace(value, axis1=1, axis2=2) - 1.0)))
        > 5e-5
    ):
...
def _parse_receipt(
    ...
    if not isinstance(csv_binding, Mapping) or not isinstance(
        npz_binding, Mapping
    ):
        ...
        for raw in reader:
            ...
            if terminal:
                ...
                if quantization_text:
                    raise RuntimeError(
                        "nonterminal density certificate must be blank"
                    )
                quantization = None
            rows.append(
                {
    ...
    with np.load(io.BytesIO(npz_payload), allow_pickle=False) as archive:
        if set(archive.files) != {"density_row_ids", "densities"}:
    ...
    if len(density_ids) != len(set(density_ids)) or len(density_ids) != len(
        density_stack
    ):
        raise RuntimeError("raw density row alignment drift")
    _physicality(density_stack, int(cell["cutoff"]))
    terminal_ids = {
        str(row["row_id"]) for row in rows if row["terminal_round"]
    }
    if set(density_ids) != terminal_ids:
        raise RuntimeError("raw terminal density coverage drift")
...
def _selected_reference_receipts(
    ...
    for receipt, receipt_binding in zip(
        receipts, receipt_bindings, strict=True
    ):
        cell = receipt.get("cell", {})
...
def _load_evidence(
    ...
    config, base = pilot_runner.load_pilot_config(
        root, require_hardened=True
    )
    ...
    pilot_runner._verify_complete_marker(
        root, config, run_identity, manifest
    )
    if (
    ...
    reference_manifest, reference_receipts = _selected_reference_receipts(
        root, config
    )
    receipt_pairs = list(
        zip(
    ...
    for receipt, receipt_binding in [*reference_receipts, *receipt_pairs]:
        chunk_rows, chunk_densities = _parse_receipt(
            root, receipt, receipt_binding
        )
...
def _tail_features(stack: np.ndarray, cutoff: int) -> dict[str, np.ndarray]:
    ...
    if (
        ...
        or float(np.max(np.abs(np.sum(populations, axis=1) - 1.0)))
        > 5e-5
    ):
...
def _fault_positions(
    grouped: Mapping[
        tuple[int, str, str, str, int], Sequence[Mapping[str, Any]]
    ],
    cutoff: int,
    scenario: str,
    backend: str,
...
def _fault_stage(
    ...
    *,
    ...
    positions = _fault_positions(
        grouped, cutoff, scenario, backend, state
    )
    ...
    for position in positions:
        values = [
            row
            for row in grouped[
                (cutoff, scenario, backend, state, position)
            ]
...
def _fault_terminal(
    ...
    *,
    ...
    stack = []
    quantization = []
    for position in _fault_positions(
        grouped, cutoff, scenario, backend, state
    ):
...
def _shared_index(
    ...
    for row in rows:
        ...
        grouped.setdefault(
            (int(row["cutoff"]), str(row["backend"])), []
        ).append(dict(row))
    expected = {
        (cutoff, backend)
        for cutoff in (28, 32, 36)
...
def _density_point(
    ...
    embedded = np.asarray(
        [
            _embed_density(matrix, lower_cutoff, upper_cutoff)
            for matrix in lower_stack
        ]
    )
    return _trace_distance(
        np.mean(embedded, axis=0), np.mean(upper_stack, axis=0)
    )
...
def _scalar_points(left: np.ndarray, right: np.ndarray) -> dict[str, float]:
    ...
    return {
        ...
        "level_probability_l1": float(
            np.sum(np.abs(left_mean[1:4] - right_mean[1:4]))
        ),
...
def evaluate(
                ...
                for state in states:
                    ...
                    output.append(
                        _gate(
                            ...
                            quantization=float(
                                np.mean(lower_q) + np.mean(upper_q)
                            ),
                            margin=float(
                                contract["design_density_point_threshold"]
                            ),
                            clusters=12,
                        )
                    )
        ...
        for backend in ("A", "B"):
            ...
            output.append(
                _gate(
                    ...
                    backend=backend,
                    estimate=_density_point(
                        lower_stack, upper_stack, lower, upper
                    ),
                    quantization=float(
                        np.mean(lower_q) + np.mean(upper_q)
                    ),
            ...
            for metric, estimate in _scalar_points(
                ...
                output.append(
                    _gate(
                        gate_id=(
                            f"shared/scalar/{metric}/"
                            f"{lower}->{upper}/{backend}"
                        ),
    ...
    for backend in ("A", "B"):
        stack, _ = shared_stack(36, backend)
        for metric, values in _tail_features(stack, 36).items():
            output.append(
                _gate(
...
def _report(
    ...
    for row in gates:
        family = str(row["family"])
        ledger = family_counts.setdefault(family, {"total": 0, "passed": 0, "failed": 0})
        ledger["total"] += 1
        ledger["passed" if row["passed"] else "failed"] += 1
    config_binding = _binding(
        root / pilot_runner.PENDING_CONFIG_PATH, root
    )
    report: dict[str, Any] = {
        "task_id": TASK_ID,
        ...
        "maximum_margin_ratio": max(
            float(row["conservative_point"]) / float(row["margin"])
            for row in gates
        ),
        ...
        "bindings": {
            ...
            "released_child": _binding(
                root / pilot_runner.CONFIG_PATH, root
            ),
            "release_receipt": _binding(
                root / pilot_runner.RELEASE_RECEIPT_PATH, root
            ),
            ...
            "scalar_uq_independent_verification": dict(
                config["uq_preflight_sources"]["scalar"][
        ...
        "reference_manifest_analysis_sha256": reference_manifest[
            "analysis_sha256"
        ],
...
def write_artifacts(root: Path | None = None) -> dict[str, Any]:
    ...
    config, manifest, reference, rows, densities, raw_bindings = (
        _load_evidence(repository)
    )
    lock_path = repository / str(
        config["artifact_paths"]["diagnostic_owner_lock"]
    )
    report_path = repository / str(
        config["artifact_paths"]["diagnostic_report"]
    )
    source_path = repository / str(
        config["artifact_paths"]["diagnostic_source_data"]
    )
    ...
    with _exclusive_lock(lock_path):
        if any(path.exists() for path in (report_path, source_path, completion_path)):
            if not all(
                path.exists()
                for path in (report_path, source_path, completion_path)
            ):
            ...
            if (
                ...
                != _binding(source_path, repository)
        ...
        completion: dict[str, Any] = {
            ...
            "design_manifest": _binding(
                repository
                / str(config["artifact_paths"]["execution_manifest"]),
                repository,
            ),
            "qualified_claim": None,
            "claim_state": dict(CLAIM_BOUNDARY),
        ...
        _atomic_bytes(
            ...
            (
                json.dumps(
                    completion, ensure_ascii=False, indent=2, sort_keys=True
                )
                + "\n"
            ).encode("utf-8"),
        )
