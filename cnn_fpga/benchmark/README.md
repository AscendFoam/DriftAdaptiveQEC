...
| [phase9_baseline_search_power_registry.py](phase9_baseline_search_power_registry.py) | T9.1.4 | 冻结 31 方法、18 个 mandatory matched baseline、四类不可混排排名身份、同定点 LLR/action-conditioned CRN/compute/selection/failure 合同、808 clusters/backend 的 round-LER 设计功效及 12-work 文献去重账本；36/36 gates/mutations，所有当前性能、external-SOTA、official/Puviani 与 physical claim 保持 null |
| [phase9_scoped_claim_amendment.py](phase9_scoped_claim_amendment.py) | T9.1.5 | additive v2 child seal：严格绑定 immutable T9.1.1/T9.1.4 与 canonical release pin，动态覆盖29个 legacy outputs并拆分9个 scoped states；generic evaluator非促销，strict SHA/count/threshold types、recorded/live HIL、17 forbidden transfers、exact-closure/sequence/previous-entry hash chain与externally pinned snapshot anchor的可执行revocation、27全-null predicate+4 propagation fixtures、127-row full-analysis Source Data与36/36 gates/mutations，所有当前性能与claim保持typed null |
| [phase9_causal_twin_contract.py](phase9_causal_twin_contract.py) | T9.2.1 | parent-bound pre-backend contract generator/verifier：绑定五 namespace、observed-only assembler、24-bit K、纯 `F(K)`、完整 N/T/lossless-composition totality、80-bit semantic+CRC action、previous-K receipt、14级 fault priority、trusted complete-package nomination、16 executable non-codebook probes、246-row lossless Source Data与canonical release pin；40/40 gates/mutations，全部 physics/performance/rank/official/Puviani/SOTA 字段保持 null |
| [phase9_backend_a_qualification.py](phase9_backend_a_qualification.py) | T9.2.2 | joint finite-Fock oscillator × qutrit backend-A generator/verifier：真实 GKSL/Choi、Ramsey syndrome、continuous-IQ Kraus backaction、reset/f-state、action-conditioned leakage/drift、六态 evaluator、seed/step/cutoff 收敛；20/20 gates、20/20 mutations、全部 dependencies/tests 与 T9.2.1 parent hash-bound。只资格化 backend A，backend B、dual agreement、性能与外部 claim 均为 null |
| [phase9_backend_b_qualification.py](phase9_backend_b_qualification.py) | T9.2.3 | 独立 backend-B generator/verifier：dense midpoint Strang + 解析 Kraus/Schur、BLAKE2b/Python Random/Box–Muller、独立 IQ likelihood/Kraus 与 squeezed-comb evaluator；AST/token隔离、闭式 loss/relaxation、CP/TP/full physicality、action/leakage/drift、seed/split/cutoff，22/22 gates/mutations，全部 source/tests 与 T9.2.1/T9.2.2 evidence hash-bound。只资格化 backend B，A/B agreement、性能和外部 claim 均为 null |
| [phase9_twin_qualification_contract.py](phase9_twin_qualification_contract.py) | T9.2.4 prereg | formal-outcome-free A/B qualification seal：hash-bind T9.2.1--T9.2.3，冻结六态/七动作/五共同初态/16 probes/四故障family、14,976 backend-round、384 trajectories、互斥seed、18 tolerance、simultaneous family与NO-GO/no-retune/no-delete/no-mean-rescue；20/20 gates/mutations，性能与外部claim全null |
| [phase9_twin_formal_runner_amendment.py](phase9_twin_formal_runner_amendment.py) | T9.2.4 additive prereg | 只增量绑定 immutable parent、解析 Hermite/Mehler bridge、formal runner、独立 verifier、测试与LFS原始态归档；冻结总16,800 backend-round、19 estimands、2,000次max-T bootstrap、零postselection及三态fail-closed，30/30 gates/mutations，formal outcome未访问 |
| [phase9_dual_backend_qualification.py](phase9_dual_backend_qualification.py) | T9.2.4 formal runner | 一次性消费 parent+child seal，执行六态/共同物理态/动作/故障/cutoff完整矩阵；异常不删行，保存逐行seed receipt、trace-decreasing logical block与raw density/isometry NPZ；禁止CLI覆盖正式seed/tolerance |
| [phase9_dual_backend_verifier.py](phase9_dual_backend_verifier.py) | T9.2.4 independent verifier | 不导入physics模块，从ledger/raw archive独立重算1,042类cell/gate metric、unconditional PTM、fault terminal与within-backend cutoff；应用冻结cluster max-T CI并签发PASS、科学NO_GO或基础设施INCOMPLETE |
| [phase9_dual_backend_post_outcome_audit.py](phase9_dual_backend_post_outcome_audit.py) | T9.2.4 post-outcome audit | 不导入physics或formal verifier，直接审计16,800-row ledger、44 MB NPZ、1,538-row Source Data、release pin与self-hash，独立重算最差log-evidence点、825/217 gate计数、失败分布、零postselection和NO-GO传播；25/25 gates、24/24 mutations，只确认科学NO-GO，不开放任何性能claim |
