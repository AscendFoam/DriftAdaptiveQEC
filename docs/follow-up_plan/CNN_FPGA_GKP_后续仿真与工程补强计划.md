...
## 16. 参考 NVIDIA 论文后的可吸收改进

### 16.1 总体判断

新增论文 [Fast and accurate AI-based pre-decoders for surface codes.md](../relative_papers/Fast%20and%20accurate%20AI-based%20pre-decoders%20for%20surface%20codes.md) 对当前项目有明确启发，但更适合作为方法论参考，而不是直接照搬网络结构。

原因是：

1. 论文的主问题是 `surface code + global decoder` 的预解码与权重学习。
2. 当前项目的主问题是 `GKP + FPGA 快回路 + 慢回路自适应` 的闭环控制。
3. 论文中的 learned module 更像“统计估计器 / pre-decoder”，当前主线 `hybrid_residual_b` 更像“teacher-guided residual controller”。

因此，最合理的做法不是把论文网络直接搬到当前项目，而是吸收它背后的设计原则。

### 16.2 可以直接借鉴的 5 条方法论

结合论文和当前项目，最值得借鉴的是以下 5 条方法论：

1. 训练目标要尽量贴近下游真正关心的量，而不是停留在中间参数拟合。
2. 全局统计量更适合走低维聚合支路，而不是粗暴广播成整张平面通道。
3. 稳定性最好在模型内表达一部分，而不是完全依赖外部 EMA。
4. 可识别性优先于模型容量，不能从当前统计量稳定识别出的量，不应强行让 CNN 去学。
5. learned module 的角色要说清楚，是“校准器 / 残差补偿器”，而不是“完全替代解码器”。

这 5 条里，前两条与当前 `teacher params` 重编码主线最直接相关。

### 16.3 与当前项目已经形成的判断高度一致的地方

论文中最值得当前项目吸收的一点，是它不把“全局噪声信息”粗暴地广播成整张图像，而是通过低维聚合路径来表达统计状态。

这与当前项目已经观察到的现象高度一致：

1. `teacher params` broadcast 容易带来冗余耦合。
2. 低维 `scalar_branch + gated` 比整包平面广播更合理。
3. 只保留少数高价值 teacher 标量的 `Gated v5` 已经在 formal HIL 中稳定优于 `Full`。

因此，当前项目在 `Gated v5` 上做出的判断，并不是孤立的经验结果，而是有外部论文方法论支撑的：

- 问题不在于 `teacher` 信息整体有害。
- 问题更像是“整包 teacher 标量 + broadcast 编码”不适合当前闭环任务。

### 16.4 不能直接照搬的地方

也必须明确，下面这些内容不适合直接照搬：

1. 论文的下游是 `PyMatching / MWPM` 这类全局图解码器，当前项目下游并不是这类解码器。
2. 论文的输出更接近 edge / hyperedge 权重或其统计替身，当前项目真正消费的是 `(K, b)`，尤其是 `b` 的运行时修正。
3. 论文偏 GPU 高吞吐批量并行，当前项目首先要满足的是 ARM/FPGA 双回路的部署时延与运行时语义。
4. 论文里更大的 2D/3D CNN 结构不能直接作为当前项目默认主线，否则会压过现有已经验证有效的轻量部署路线。

因此，当前不建议：

1. 不建议把当前主线重新改回“完整绝对噪声参数回归”。
2. 不建议为了模仿论文而引入过重的大模型，打断 `Gated v5` 主线。
3. 不建议把项目叙事改写成“AI 完全替代解码器”。

### 16.5 对当前主线最有价值的 4 个改进方向

#### 16.5.1 方向 A：把 loss 再往“闭环有效量”推进一步

当前 `residual_b` 已经比早期 `(sigma, mu_q, mu_p, theta)` 绝对参数回归更贴近下游闭环，这条路是正确的。  
但和论文相比，我们仍主要是在做“监督残差回归”，还可以再往前一步。

建议在保留当前 `delta_b` 监督的基础上，加入一类轻量的闭环一致附加损失，例如：

1. `b_next = teacher_b + delta_b` 与目标 `target_b` 的误差。
2. 一步校正后的残余风险代理量。
3. `overflow / saturation` 风险的可微近似项。

这样做的意义是：

1. 保持当前 runtime-consistent 数据链路不变。
2. 让模型不只学“标签像不像”，还学“这次补偿对闭环有没有用”。
3. 更接近论文那种“通过结构化中间层训练真正有用量”的思想。
...
这已经比单窗口方案强很多，但论文提醒我们：  
如果一个量本质上是“慢变化统计状态”，稳定性最好在模型内也表达一部分，而不是完全交给后处理。

因此建议在现有轻量 `tiny_cnn` 路线里，尝试一个很克制的增强版本：

1. 保留当前多窗口 histogram 输入不变。
2. 保留 `scalar_branch + gated` 不变。
3. 在卷积主干后增加一个更明确的窗口聚合或轻量 pooling/attention 头。

这条线的目标不是复制论文的大模型，而是验证：

- 当前 `Gated v5` 的稳定性，有多少来自模型本身的统计表达，有多少只是依赖外部 EMA。

#### 16.5.3 方向 C：把慢回路叙事明确拆成“状态估计器 + 残差控制器”

这是论文对当前项目最值得借的 framing。

当前主线实际上已经形成了两层结构：

1. `Window Variance / EKF / UKF`
   - 给出稳定的慢漂移统计状态
2. `CNN residual-b`
   - 在 teacher 基础上做小幅、任务相关的闭环修正

但这件事在当前文档和实验设计里还可以写得更清楚。  
后续建议把它显式表述为：
...
1. 更方便与外部论文对话。
2. 更容易解释为什么当前主线不是“让 CNN 替代一切”。
3. 更有利于后续模块化替换 teacher 或 residual controller。

这条改进短期内不一定要立刻改代码，但应进入后续文档与实验设计口径。

#### 16.5.4 方向 D：把“可识别性检查”变成显式前置步骤

论文一个非常重要的隐含启发是：  
不是所有想学的量，都能从当前 syndrome 统计里被稳定识别。

这与当前项目早期 `theta` 学不稳，以及后续 `teacher params` 冗余问题，本质上是同类问题。

因此，后续所有新的 teacher 表征尝试前，都建议先补一个显式的“可识别性检查”：

1. 当前窗口统计对该量是否足够敏感。
2. 该量是否能被其它已有输入高精度重构。
3. 它到底是新增信息，还是已有信息的改写版本。

只有通过这一步的量，才值得继续进入 CNN 输入设计。

### 16.6 建议新增一条“论文启发分支”，但不要压过当前 Gated v5 主线

结合当前结果，最合理的做法不是把论文变成新的唯一主线，而是新增一条受控分支：

- `paper_inspired_statcalib`

这条分支的定位应当是：

1. 不替代当前 `Hybrid Full` 正式主线。
2. 不替代当前 `Gated v5` 作为 teacher-representation 主候选。
3. 只作为下一阶段结构增强分支，用来验证论文启发是否能把 `Gated v5` 再往前推一步。

建议这条分支的最小实现版本包含：

1. 输入
   - 保留当前多窗口 histogram
   - 保留 `teacher_b_q / teacher_b_p / teacher_delta_b_q / teacher_delta_b_p`
   - 不恢复整包 broadcast teacher 通道

2. 模型
   - 继续使用轻量 CNN 主干
   - scalar branch 继续 gated
   - 增加一个轻量统计聚合头

3. 输出
   - 仍输出 `delta_b_q / delta_b_p`
   - 不回退到完整绝对物理参数输出
...
- 验证“论文的方法论”能否在不破坏当前工程主线的前提下，进一步提升 formal HIL 稳定性。

### 16.7 纳入当前里程碑的位置

这部分内容不应排到最前面，也不应无限后置。  
结合现有计划，建议它的优先级如下：

1. 低于当前 `Gated v5` 的更长 paired benchmark 与更稳定复验。
2. 低于第一版 runtime/latency/fault 注入增强。
3. 高于纯叙事型、但短期不改变实验结论的板级语义补强。

可以把它插入为一个中间里程碑：

- `M1.5：paper-inspired closed-loop calibration branch`

建议交付物为：

1. 一份新的配置模板
2. 一轮小规模 paired benchmark
3. 一份与 `Gated v5` 的对照报告

### 16.8 最终判断

这篇 NVIDIA 论文对当前项目最值得借的，不是它的具体网络外形，也不是 surface code 的任务设定，而是下面这条更一般的方法论：

- 让 learned module 去学习“对下游解码真正有用的有效统计量或残差”
- 把统计聚合、物理先验和部署语义一起写进模型

从这个角度看，当前项目从“绝对参数回归”走到“teacher-guided residual-b”，方向本身是对的。  
后续真正值得补的，不是回到更早的参数回归路线，而是：

1. 继续保留 `residual-b` 这条下游一致主线
2. 继续坚持低维 teacher scalar branch，而不是整包 broadcast
3. 在此基础上引入更显式的统计聚合与闭环一致 loss

如果按这个思路推进，那么论文给当前项目带来的最大收益，不是“换模型”，而是让当前方法更容易被表述为：

- `a hardware-aware learned calibration / residual correction module for real-time decoder adaptation under drift`

这与当前双回路、teacher-guided、formal HIL 和 FPGA 部署叙事，是高度一致的。
