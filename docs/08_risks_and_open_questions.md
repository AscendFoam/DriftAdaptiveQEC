...
| R33 | 当前暂无 `Linux + FPGA` 硬件宿主，使得真板 execution 路线既缺资源条件，也容易打断主线 paper-material 节奏 | 高 | 2026-06-11 用户已明确当前暂无 `Linux + FPGA` 硬件；`T49/T71/T72` 的 strongest supported truth 仍只是 current-host `NO_GO`、read-only regeneration 与 provenance hardening，尚未出现任何真板 execution evidence | 保持 `T37` 为 `blocked + lowest-priority backlog`；main 分支当前优先推进 `T76/T77` 这类 paper-facing rendered-QA / note-sync / traceability-hardening tasks；仅在硬件条件变化后再重开真板 execution 准备 |
| R34 | `T76` 已完成真实 rendered QA 与 Results assembly，但 preview-source schema 与逐图 stable-ID 绑定仍偏间接：聚合预览行复用了 `upstream_t74_ids` 字段语义，逐图 QA 结论也未把所有上游 `T74-*` stable IDs 内联写全；若直接把当前 `T76` 预览包复用于 note-draft 同步或后续自动化，容易让 paper-facing traceability 粒度不够干净 | 中 | `docs/review/T76_review.md`; `docs/figure_assets/T76_rendered_figure_qa_pack/render_manifest.json`; `docs/figure_assets/T76_rendered_figure_qa_pack/preview_source_map.csv`; `docs/paper_materials/paper_rendered_figure_qa.md`。当前 traceability 仍然成立，但依赖 `render_manifest.json` 与 source-map 的间接组合，而不是逐图显式内联；review 同时指出缺少一条很轻量的 schema 级检查 | 保持 `T76` 作为已完成的 rendered-QA / assembly 任务，但不要把当前 preview-source 结构当成长期通用 schema；下一张 bounded follow-up 应拆清 `source_preview_ids` / `upstream_t74_ids` 语义，在逐图 QA 结论里显式写全上游 stable IDs，并把经修补后的 traceability 同步到 `docs/paper_notes/*.tex` 的结果层 |
...
- The authoritative task package is `docs/tasks/Phase2/T77_paper_note_results_sync_and_traceability_hardening.md`.
- `T76` has been accepted as `PASS_WITH_WARNINGS`.
- `R30` is closed by `T71`; `R31` is closed by `T72`; `R32` remains the narrowed deployment-boundary carry-forward risk; `R34` is the new paper-facing traceability carry-forward risk from `T76`.
...
- 当前唯一任务：`T77: 论文 note-draft 结果层同步与 T76 traceability hardening`，任务包 `docs/tasks/Phase2/T77_paper_note_results_sync_and_traceability_hardening.md`；它只允许在 `T74/T75/T76` 已锁定的结果层材料边界内做 note-draft 同步、preview-source schema 清理、逐图 stable-ID 显式绑定与可选编译检查，不得扩到 benchmark、HIL、真板成功宣称、full-manuscript reopen、theory branch 大范围改写、sidecar promotion 或 `T37` 真板执行。
...
9. 下一张继续开发任务包应该优先选哪一类？
   - 当前答案：
     ...
     - 当前唯一任务已切换为 `T77: 论文 note-draft 结果层同步与 T76 traceability hardening`，任务包已存在：`docs/tasks/Phase2/T77_paper_note_results_sync_and_traceability_hardening.md`；它要求继续补 note-draft 结果层同步、preview-source schema 清理、逐图 stable-ID 显式绑定，以及在本地工具可用时做受控 note 编译检查。
