...
| R24 | 当前 `statcalib` lane 仍是最小 comparator 语义；若把 T30 helper、T59/T60/T62 smoke lane 或 T64 extension-lane bounded win 误当完整 statcalib/calibration comparator，可能把受限接口与 mock-backed lane 外推成未验证算法能力 | 中 | `docs/review/T30_review.md` N4；`docs/review/T59_review.md`；`docs/review/T60_review.md`；`docs/review/T62_review.md`；`docs/review/T64_review.md`。T64 证明 extension lane 可在 locked protocol 下干净执行并赢过 frozen winner，但其参数仍沿用 earlier smoke lane 的最小 comparator 语义，而不是独立 calibration search 或成熟 comparator validation | 后续 FR8 文档、图表与 gate 必须继续把 `statcalib` 写成 separately labeled extension lane；不得把 T64 bounded result 外推为完整 calibration comparator、`.tflite` 或 real-board 证据；在 `T65` 完成前也不得把 T64 result pack 视为自校验 report pack |
...
| R28 | T64 result-pack 的 report/artifact consistency 仍依赖人工整理；当前报告存在执行形态措辞漂移与 finish timestamp provenance 误归因，而且仓库还缺一个 extension-lane report 对照 artifact 的自动护栏 | 中 | `docs/review/T64_review.md`；`docs/fr8_statcalib_extension_lane_benchmark.md`；`runs/p4_benchmark/T64_fr8_statcalib_extension_lane_20260527_221658/summary.json`；`launch_plan.json`；`progress.jsonl`。当前 T64 主体实验结果可接受，但报告自证一致性还不够强 | `T65` 应修正 T64 报告措辞、补一个轻量 audit helper 与 focused regression test，并产出显式 consistency-audit doc；在 `T65` 完成前，不把 T64 report pack 当作自校验可复用材料 |
