...
- 当前唯一任务：`T80: 主线校准段落的 bounded prose reopen`，任务包 `docs/tasks/Phase2/T80_mainline_calibrated_sections_bounded_prose_reopen.md`；它只允许在现有 note / paper-material / claim-evidence / risk 台账边界内重写已经 ready 的 8 个 narrative / result-facing sections，不得扩到 methods chapters、benchmark、HIL、真板成功宣称、full-manuscript reopen、theory branch 大范围改写、sidecar promotion 或 `T37` 真板执行。
...
9. 下一张继续开发任务包应该优先选哪一类？
   - 当前答案：
     ...
     - `T79` 已完成并由 Captain 接受为 `PASS`；它已把当前 note / results pack / claim-evidence / risk 台账的 reopen/readiness 问题收口为唯一 gate 结论 `GO_FOR_BOUNDED_PROSE_REOPEN`。
     - 当前唯一任务已切换为 `T80: 主线校准段落的 bounded prose reopen`，任务包已存在：`docs/tasks/Phase2/T80_mainline_calibrated_sections_bounded_prose_reopen.md`；它只允许在当前已 ready 的 8 个 narrative / result-facing sections 内做受控 prose reopen，不直接恢复 full-manuscript 扩写，也不触碰方法章、benchmark、`.tflite`、real-board 或 theory 分支大范围改写。
