...
| R5 | P4 目前虽已完成 frozen baseline 单场景全模式 smoke 复验，且 `T14` 已补 development protocol，但仍没有恢复正式多场景 frozen benchmark | 中高 | `T9` 已复验 `static_bias_theta + static_linear/window_variance/ekf/cnn_fpga + repeats=1`，run dir 为 `runs/p4_benchmark/p4multis_20260508_121828_0c12d7_46732`；`T14` 已把下一步限定为 `static_bias_theta + linear_ramp`、五模式、`repeats=2` 的 bounded matrix | 先按 `docs/P4_benchmark_development_protocol.md` 执行 `T15` bounded run，再由 `T16` 判断是否恢复完整四场景 formal benchmark |
...
| R9 | Phase 2 若跳过 `T14` 定义的矩阵直接运行更大范围 P4 benchmark，仍可能隐式扩大计算范围或混淆 recovery/development/formal 口径 | 中高 | `docs/02_experiment_plan.md` 明确禁止无准备长跑；`T14` 已把 `T15` 限定为双场景、五模式、`repeats=2` 的 bounded run | 后续只允许在 `docs/P4_benchmark_development_protocol.md` 明示范围内执行；超出范围必须新开任务包 |
...
10. `T15` 是否应直接运行多场景 P4 smoke？
   - 当前答案：可以进入有界多场景开发级 run，但只能按 `T14` 固定的 matrix 执行：
