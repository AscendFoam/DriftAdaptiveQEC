...
| R31 | `T71` 已做出可 replay / regeneration 的 checked-in transfer pack，但 provenance 仍不够 execution-derived / override-safe：`probe_limitations` 存在未实际探测却写成事实的固定文案，`source_records` 与 `expected_byte_count_basis` 仍偏默认 config 写死，而且 `--config` / `--mmio-path` / `--dma-path` 的 focused regression 不足 | 中 | `docs/review/T71_review.md`; `docs/evidence_packs/deployment_boundary/t71_real_board_gate_regeneration_pack.md`; `cnn_fpga/hwio/collect_t71_real_board_gate_artifacts.py`（`probe_limitations`, `source_records`, `expected_byte_count_basis`）；`tests/test_t71_real_board_gate_regeneration_pack.py`。这些问题不改变当前 current-host `NO_GO` verdict，但会削弱 future-host host-transfer pack 的 provenance 严谨度，并可能让 override 情况下的说明文本与实际执行上下文脱节 | 打开 `T72`：去掉未探测即写死的 provenance 文案，让 config/path provenance 改为 execution-derived / override-aware，并补齐 `--config` / `--mmio-path` / `--dma-path` 相关 focused regression；在 `R31` 关闭前，`T37` 继续 blocked，且 downstream retelling 不得把 `T71` transfer pack 描述成长期稳定的 future-host 标准入口 |

## 当前开放问题

Authoritative supersession note:

- The current unique task for all new worker action is `T72: Real-board transfer-pack provenance hardening`.
- The authoritative task package is `docs/tasks/Phase2/T72_real_board_transfer_pack_provenance_hardening.md`.
- `T71` has been accepted as `PASS_WITH_WARNINGS`.
- `T71` warning handling is `W1/W2/W3 = deferred -> R31`, `W4 = accepted`.
- `R30` is closed by `T71`; `R31` is the new carry-forward risk.
...
- The authoritative checked-in regeneration carry-forward artifacts now also include `docs/evidence_packs/deployment_boundary/t71_real_board_gate_regeneration_pack.md` and `artifacts/t71_real_board_gate_regeneration_pack/current_host_regenerated_gate.json`.
- If older Q&A carry-forward text below still mentions `T71` or earlier tasks as current, treat it as historical text only.
