...
| R33 | 当前暂无 `Linux + FPGA` 硬件宿主，使得真板 execution 路线既缺资源条件，也容易打断主线 paper-material 节奏 | 高 | 2026-06-11 用户已明确当前暂无 `Linux + FPGA` 硬件；`T49/T71/T72` 的 strongest supported truth 仍只是 current-host `NO_GO`、read-only regeneration 与 provenance hardening，尚未出现任何真板 execution evidence | 保持 `T37` 为 `blocked + lowest-priority backlog`；main 分支当前优先推进 `T73/T74` 这类 paper-facing simulation/material tasks；仅在硬件条件变化后再重开真板 execution 准备 |

## 当前开放问题

Authoritative supersession note:

- The current unique task for all new worker action is `T73: Mainline claim/evidence and result/figure/risk ledger refresh`.
- The authoritative task package is `docs/tasks/Phase2/T73_mainline_claim_evidence_and_result_figure_ledger_refresh.md`.
- `T72` has been accepted as `PASS_WITH_WARNINGS`.
- `T72` warning handling is `N1/N3 = deferred -> R32`, `N2 = accepted`.
- `R30` is closed by `T71`; `R31` is closed by `T72`; `R32` is the new narrowed carry-forward risk.
...
- If older Q&A carry-forward text below still mentions `T72` or earlier tasks as current, treat it as historical text only.
...
- 当前唯一任务：`T73: Mainline claim/evidence and result/figure/risk ledger refresh`，任务包 `docs/tasks/Phase2/T73_mainline_claim_evidence_and_result_figure_ledger_refresh.md`；它只允许在主线 paper-facing 三台账刷新边界内工作，不得扩到 benchmark、HIL、真板成功宣称、paper reopen、theory branch、sidecar promotion 或 `T37` 真板执行。
...
9. 下一张继续开发任务包应该优先选哪一类？
   - 当前答案：
     ...
     - 当前唯一任务已切换为 `T73: Mainline claim/evidence and result/figure/risk ledger refresh`，任务包已存在：`docs/tasks/Phase2/T73_mainline_claim_evidence_and_result_figure_ledger_refresh.md`；它要求把 post-`T72` 的主线 claim/result/risk 三台账统一回写，而不是继续打开执行长跑。
     - 若 `T73` 通过，唯一推荐下一任务为 `T74: Paper-ready simulation result and figure pack`，继续补论文可直接复用的仿真结果、图表与 captions。
