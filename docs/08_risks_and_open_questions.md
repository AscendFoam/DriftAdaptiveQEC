...
| R30 | `T49` 的 current-host 真板 gate role-aware / regeneration 路径缺口 | 已收口 | `docs/review/T49_review.md`; `docs/review/T71_review.md`; `docs/t49_real_board_smoke_execution_gate.md`; `docs/t71_real_board_gate_regeneration_pack.md`; `cnn_fpga/hwio/build_t49_real_board_smoke_gate.py`; `cnn_fpga/hwio/collect_t71_real_board_gate_artifacts.py`; `tests/test_t49_real_board_smoke_execution_gate.py`; `tests/test_t71_real_board_gate_regeneration_pack.py`。`T71` 已补上 role-aware `mmio + dma` 判定、checked-in read-only collector、以及 `T49` replay / current-host regeneration 一致性回归，且两条 fresh verification verdict 都仍为 `NO_GO_REAL_BOARD_HOST_OR_DEVICE_PATH_UNAVAILABLE` | `R30` 已由 `T71` 关闭；后续不要再把它当成未解决的 role-aware/gate-regeneration 缺口 |
| R31 | `T71` 已做出可 replay / regeneration 的 checked-in transfer pack，但 provenance 仍不够 execution-derived / override-safe：`probe_limitations` 存在未实际探测却写成事实的固定文案，`source_records` 与 `expected_byte_count_basis` 仍偏默认 config 写死，而且 `--config` / `--mmio-path` / `--dma-path` 的 focused regression 不足 | 中 | `docs/review/T71_review.md`; `docs/t71_real_board_gate_regeneration_pack.md`; `cnn_fpga/hwio/collect_t71_real_board_gate_artifacts.py`（`probe_limitations`, `source_records`, `expected_byte_count_basis`）；`tests/test_t71_real_board_gate_regeneration_pack.py`。这些问题不改变当前 current-host `NO_GO` verdict，但会削弱 future-host host-transfer pack 的 provenance 严谨度，并可能让 override 情况下的说明文本与实际执行上下文脱节 | 打开 `T72`：去掉未探测即写死的 provenance 文案，让 config/path provenance 改为 execution-derived / override-aware，并补齐 `--config` / `--mmio-path` / `--dma-path` 相关 focused regression；在 `R31` 关闭前，`T37` 继续 blocked，且 downstream retelling 不得把 `T71` transfer pack 描述成长期稳定的 future-host 标准入口 |
...
- The authoritative checked-in regeneration carry-forward artifacts now also include `docs/t71_real_board_gate_regeneration_pack.md` and `artifacts/t71_real_board_gate_regeneration_pack/current_host_regenerated_gate.json`.
