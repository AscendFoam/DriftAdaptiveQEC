...
| R15 | Phase 2 当前虽然已补齐 FR8 closure pack、training/material pack 和 current-host isolated true `.tflite` runtime，但证据仍分层停留在 bounded software/runtime/readiness 层；若直接升级到 default-env runtime、physical cleanup、real-board validation 或 paper reopen，仍容易再次打破边界诚实 | 高 | `docs/review/T21_phase2_milestone_review.md` verdict = `Conditional`；`T19` 未执行 cleanup；`T20/T22` 仍不是真板验证；`T48` 只确认 isolated current-host true `.tflite` runtime，不等于 default env 或 deployment closure；`T50` 只是一份 bounded training/material pack | 保持 `Phase 2: Controlled Development` / `Go`，继续只开 bounded 下一任务；在当前暂无 `Linux + FPGA` 硬件宿主的前提下，主线先补 `T78` 这类 paper-facing note-calibration / hierarchy / layout closeout tasks，而不是直接进入高风险板级执行或 paper reopen |
| R16 | 把“最终要发论文”误压缩成最近任务直接写论文 claim，容易跳过 formal benchmark、机制诊断和部署边界证据 | 高 | `T24` 已完成 frozen-set formal software revalidation，`T25` 已确认其 result boundary，`T27/T28` 已缩窄并修复 teacher diagnostics 输出语义，`T29` 已修复人读 report header，`T26` 已完成 statcalib feasibility gate，`T48` 已确认 isolated true `.tflite` runtime；但 `T48` 仍不是 default-env / deployment closure，`T20/T22` 仍不是 hardware validation，`R10` 仍不是完整机制证据 | Paper claim/evidence ledger 与后续 prose reopen 仍应滞后于证据材料本身；当前主线先做 `T78` 这类受控 note 非结果层校准、hierarchy 降权和 layout closeout 任务，再决定是否恢复 prose，而不是直接升级论文 claim |
...
| R35 | `T77` 虽已完成 note 结果层同步与 traceability hardening，但当前 `docs/paper_notes/CNN_FPGA_GKP_theory_note_draft.tex` 仍只是“结果层已校准”的局部完成态：整份 `.tex` 还保留未校准的非结果层历史段落，`statcalib` 在 `Numerical Results` 中的视觉层级仍偏高，`.log` 仍有 `Underfull \\hbox` warning，而且 section-scope 证明仍主要依赖 manifest / `% T77-SOURCE` 注释而非更机械的审计 | 中 | `docs/review/T77_review.md`; `docs/paper_materials/paper_note_results_sync_manifest.md`; `docs/paper_notes/CNN_FPGA_GKP_theory_note_draft.tex`; `docs/paper_notes/CNN_FPGA_GKP_theory_note_draft.log`。当前 strongest supported truth 是“结果层已同步、整份 note 尚未 fully calibrated”，因此若直接重开 manuscript prose 或对外转述，容易把局部收口误讲成整体校准完成 | 保持 `T77` 结果层同步结论成立，但在 `T78` 完成前，不要把整份 `CNN_FPGA_GKP_theory_note_draft.tex` 当作 fully calibrated note；下一张 bounded follow-up 应完成非结果层校准、`statcalib` 层级降权、LaTeX warning 收口与更机械的 section-scope 审计 |
...
- The authoritative task package is `docs/tasks/Phase2/T78_paper_note_alignment_statcalib_hierarchy_and_layout_closeout.md`.
- `T77` has been accepted as `PASS_WITH_WARNINGS`.
- `R30` is closed by `T71`; `R31` is closed by `T72`; `R32` remains the narrowed deployment-boundary carry-forward risk; `R34` is closed by `T77`; `R35` is the new paper-facing note-calibration carry-forward risk.
...
- 当前唯一任务：`T78: 论文 note-draft 非结果层校准、statcalib 层级降权与排版 warning 收口`，任务包 `docs/tasks/Phase2/T78_paper_note_alignment_statcalib_hierarchy_and_layout_closeout.md`；它只允许在现有 note / paper-material 边界内做非结果层校准、`statcalib` hierarchy 降权、LaTeX 排版收口与更机械的 section-scope 审计，不得扩到 benchmark、HIL、真板成功宣称、full-manuscript reopen、theory branch 大范围改写、sidecar promotion 或 `T37` 真板执行。
...
9. 下一张继续开发任务包应该优先选哪一类？
   - 当前答案：
     ...
     - 当前唯一任务已切换为 `T78: 论文 note-draft 非结果层校准、statcalib 层级降权与排版 warning 收口`，任务包已存在：`docs/tasks/Phase2/T78_paper_note_alignment_statcalib_hierarchy_and_layout_closeout.md`；它要求继续补 note 的非结果层校准、`statcalib` hierarchy 降权、LaTeX 排版 warning 收口，以及更机械的 section-scope 审计。
