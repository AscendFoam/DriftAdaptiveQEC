...
- The authoritative task package is `docs/tasks/Phase2/T90_training_same_host_repeated_run_consistency_pack.md`.
...
- `R11/R13/R14/R32/R33` remain open.
- `R15`/`R16`/`R33` 的当前 mitigation 应读作：主线现阶段已从 frozen-mainline handoff 收口切回一条非硬件 evidence-hardening lane，即 `T90` 的 same-host repeated-run consistency，而不是继续停留在 `T89`。
- The theory branch remains isolated from this mainline task.
- If older Q&A carry-forward text below still mentions `T89` or earlier tasks as current, treat it as historical text only.
...
| R2 | 根目录虽已补 recovery-scoped manifest，但完整训练链、`.tflite` 与真板环境仍无统一依赖说明 | 中 | `requirements-recovery.txt` 只覆盖 `P0/P3/P4 recovery smoke`，且显式不含 `torch`、`tensorflow`、`tflite-runtime`；`docs/evidence_packs/training_reproducibility/training_chain_bootstrap.md` 已补训练链 bootstrap，但还不是跨机器完整依赖锁定 | 继续保持作用域诚实；训练链已独立说明，`.tflite` 与真板路径仍需单开有界 manifest / bootstrap 任务 |
...
| R7 | `T19` manifest 对应的 tracked-cache physical cleanup execution | 已收口 | `T33` 已按 `docs/evidence_packs/repo_hygiene/cleanup_tracked_cache_manifest.md` 对 9 个 manifest-listed `__pycache__` 目录执行 bounded untrack；`git ls-files | rg "__pycache__|\\.pyc$"` 已归零 | `R7` 对 tracked-cache lane 已关闭；`runs/` / `artifacts/` 如需处理仍必须另开任务 |
...
| R9 | T24 已完成 formal frozen-set revalidation，但若继续扩大到更多 repeat、CI-driven stopping 或 extra drift families，仍可能隐式越过 frozen-set/formal 边界 | 中 | T24 已按 locked protocol 跑完 `4 scenarios x 5 modes x repeats=2`；`docs/protocols/benchmark/P4_benchmark_formal_protocol.md` 已锁定边界 | T24 后仍不应自动追加更大 repeat、额外 scenario 或 statcalib comparator；任何进一步 P4 扩展都必须新开任务包 |
| R10 | `hybrid_residual_b` 的机制证据虽然已经补到 multi-seed diagnostic generalization 与一次 bounded targeted intervention evidence，但当前机制解释仍不能闭环，因为 `T55` 表明“high committed-`b` is harmful”并不是可泛化的一般解释，后续需要 claim reframing 与更清晰的 causal isolation | 中 | `docs/review/T27_teacher_diagnostics_path_audit.md` 已将主因缩窄为 broadcast teacher features 不触发 scalar explain diagnostics；`docs/review/T28_review.md` 确认当前输出已显式标记 `not_generated` / `not_applicable`；`docs/evidence_packs/mechanism_ablation/seed20260429_failure_diagnosis.md` 与 `docs/evidence_packs/mechanism_ablation/seed20260429_trace_export_diagnosis.md` 已把 `20260429` 缩窄为 trace-supported committed combined-`b` instability；`docs/evidence_packs/mechanism_ablation/multi_seed_trace_generalization_probe.md` 与 `docs/review/T54_review.md` 已确认该模式在 6-seed pack 中为 broadly repeated with qualifications；`docs/evidence_packs/mechanism_ablation/multi_seed_i1_intervention_probe.md` 与 `docs/review/T55_review.md` 已表明 pure I1 lower-clip intervention 为 mixed 且整体偏 harmful；`docs/review/T56_review.md` 已把机制叙事明确收口为 retain / weaken / retire / reframe / still-open | R10 remains open but changes character after `T56`：问题不再是缺 intervention evidence，而是已有 intervention evidence 使简单 harmful-instability 叙事站不住脚。`T56` 已完成 claim reframing；当前 `T47` 只能作为 hedge-conditioned paper-material lane 推进，不能写成机制闭环或第二个 intervention 自动开启 |
| R11 | 训练链 clean CPU-only environment 已完成 draft lock、dry-run/import bootstrap、one real-training smoke、one bounded real train+eval rerun 与一份 code-backed material-regeneration pack，但 full training reproducibility 与 broader portability 仍未验证 | 中 | `docs/evidence_packs/training_reproducibility/training_chain_portable_dependency_lock_plan.md` 已由 T31 产出并通过 `docs/review/T31_review.md` `PASS`；`requirements-train-cpu-win-py312.txt` 与 `docs/evidence_packs/training_reproducibility/training_chain_cpu_cleanenv_bootstrap.md` 已由 T39 产出并通过 `docs/review/T39_review.md` `PASS`；`docs/evidence_packs/training_reproducibility/training_chain_cpu_cleanenv_train_smoke.md` 已由 T40 产出并通过 `docs/review/T40_review.md` `PASS`；`docs/evidence_packs/training_reproducibility/training_reproducibility_and_material_regeneration_pack.md` 与 `docs/review/T50_review.md` 已确认当前仓库已有 clean CPU-only bounded train+eval rerun 与统一 training/material pack；clean env 与 `DLEnv` 分离 | 不把 `T50` 写成 full training reproducibility、repeated-run closure、cross-host / GPU/CUDA / Linux portability、`.tflite` correctness 或 production-scale training validation proof；当前下一步已固定为 `T90` 的 same-host repeated-run consistency pack，用来补强 repeated-run 层证据，但即便 `T90` 完成也仍不自动等于 cross-host / GPU / Linux portability closure |
...
| R15 | Phase 2 当前虽然已补齐 FR8 closure pack、training/material pack、current-host isolated true `.tflite` runtime，以及 `T82/T83/T84/T85/T86/T87/T88/T89` 的 supporting-material、全文一致性、final polish、submission-preflight、submission-pack assembly、author-final QA、manual-finish / surface-freeze 与 frozen-mainline handoff / change-control 收口，但证据仍分层停留在 bounded software/runtime/readiness 层；若直接升级到 default-env runtime、physical cleanup、real-board validation 或 submission-ready 完成态，仍容易再次打破边界诚实 | 高 | `docs/review/T21_phase2_milestone_review.md` verdict = `Conditional`；`T19` 未执行 cleanup；`T20/T22` 仍不是真板验证；`T48` 只确认 isolated current-host true `.tflite` runtime，不等于 default env 或 deployment closure；`T50` 只是一份 bounded training/material pack；`T79` 给出了 `GO_FOR_BOUNDED_PROSE_REOPEN`；`T80` 已完成 ready sections 的有界 prose reopen；`T81` 已完成 contribution/methods calibration；`T82` 已完成 supporting-boundary 四层收口；`T83` 已给出 `GO_FOR_BOUNDED_FINAL_POLISH_ONLY`；`T84` 已完成 bounded reader-facing final polish / assembly；`T85` 已完成 submission-readiness preflight 并关闭 `R36`；`T86` 已完成 bounded submission-pack assembly / exclusion route 收口；`T87` 已完成作者终检 / pre-submission QA 收口；`T88` 已完成 bounded manual finish / surface-freeze 收口并给出 `GO_FOR_FROZEN_MAINLINE_HANDOFF_ONLY`；`T89` 已完成 frozen-mainline handoff / source-of-truth / change-control 收口 | 保持 `Phase 2: Controlled Development` / `Go`，继续只开 bounded 下一任务；在当前暂无 `Linux + FPGA` 硬件宿主的前提下，主线当前进入 `T90` 这类 same-host reproducibility hardening，而不是直接进入 default-env runtime、真板执行或 submission-ready 完成态 |
| R16 | 把“最终要发论文”误压缩成最近任务直接写论文 claim，容易跳过 formal benchmark、机制诊断和部署边界证据 | 高 | `T24` 已完成 frozen-set formal software revalidation，`T25` 已确认其 result boundary，`T27/T28` 已缩窄并修复 teacher diagnostics 输出语义，`T29` 已修复人读 report header，`T26` 已完成 statcalib feasibility gate，`T48` 已确认 isolated true `.tflite` runtime；但 `T48` 仍不是 default-env / deployment closure，`T20/T22` 仍不是 hardware validation，`R10` 仍不是完整机制证据，`T79` 只给出了 bounded prose reopen gate，`T80` 只完成了 ready sections 重写，`T81` 只完成了 contribution/methods calibration，`T82` 只完成了 supporting-boundary 收口，`T83` 只完成了全文一致性 gate，`T84` 只完成了有界 reader-facing final polish / assembly，`T85` 只完成了 submission-readiness preflight，`T86` 只完成了 bounded submission-pack assembly / exclusion route 收口，`T87` 只完成了作者终检 / pre-submission QA 收口，`T88` 只完成了 bounded manual finish / surface-freeze 收口，`T89` 只完成了 frozen-mainline handoff / change-control 收口 | Paper claim/evidence ledger 与后续 prose/supporting-material calibration 仍应滞后于证据材料本身；当前主线虽已从 `T79` 进入 `T80`、`T81`、`T82`、`T83`，再进入 `T84`、`T85`、`T86`、`T87`、`T88`、`T89`、`T90`，但 `T90` 也只能做 same-host reproducibility hardening，不能借此直接升级论文 claim、部署故事或硬件叙事 |
...
## 2026-06-14 Captain Supersession (T87 closeout)

- The current unique task for all new worker action is `T88: 主线 bounded manual finish 执行与 surface freeze 收口包`.
- The authoritative task package is `docs/tasks/Phase2/T88_mainline_bounded_manual_finish_and_surface_freeze.md`.
- `T87` has been accepted as `PASS`.
- `T87` non-blocking notes are all accepted as operational reminders; no new deferred/rejected warning-derived risk is opened by this closeout.
- `R36` remains closed by `T85`; `R13/R14/R32/R33` remain open.
- `R15`/`R16`/`R33` 的当前 mitigation 应读作：主线现阶段进入 `T88` 的 bounded manual-finish / surface-freeze 收口，而不是继续停留在 `T87`。
- The theory branch remains isolated from this mainline task.
- If older Q&A carry-forward text below still mentions `T87` or earlier tasks as current, treat it as historical text only.
...
- `T83` opens no new warning-derived risk and closes no deployment/runtime/board risk by itself; it only completes one docs-only full-note consistency sweep plus one explicit closeout gate.
...
## 2026-06-13 Captain Supersession (T82 closeout)

- The current unique task for all new worker action is `T83: 主线 note 全文一致性收口与 manuscript closeout gate`.
- The authoritative task package is `docs/tasks/Phase2/T83_mainline_note_full_consistency_sweep_and_closeout_gate.md`.
- `T82` has been accepted as `PASS`.
- `T82` opens no new warning-derived risk and closes no deployment/runtime/board risk by itself; it only completes one docs-only supporting-boundary closeout route.
- `R13/R14/R32/R33` remain open, and `T37` remains blocked.
- If older Q&A carry-forward text below still mentions `T82` or earlier tasks as current, treat it as historical text only.

## 2026-06-12 Captain Supersession (T80 closeout)

- The current unique task for all new worker action is `T81: Summary of Contributions 与 methods-only calibration pack`.
- The authoritative task package is `docs/tasks/Phase2/T81_summary_and_methods_calibration_pack.md`.
- `T80` has been accepted as `PASS`.
- `T80` opens no new warning-derived risk and closes no deployment/runtime/board risk by itself; it only completes one docs-only section-bounded prose reopen on the 8 already-ready sections.
- `R13/R14/R32/R33` remain open.
- `T37` remains `blocked + lowest-priority backlog`.
- If older Q&A carry-forward text below still mentions `T80` or earlier tasks as current, treat it as historical text only.

## 2026-06-12 Captain Supersession (T79 closeout)

- The current unique task for all new worker action is `T80: 主线校准段落的 bounded prose reopen`.
- The authoritative task package is `docs/tasks/Phase2/T80_mainline_calibrated_sections_bounded_prose_reopen.md`.
- `T79` has been accepted as `PASS`.
- `T79` opens no new warning-derived risk and closes no deployment/runtime/board risk by itself; it only fixes the current prose-reopen gate answer to `GO_FOR_BOUNDED_PROSE_REOPEN`.
- `R13/R14/R32/R33` remain open.
- `T37` remains `blocked + lowest-priority backlog`.
- If older Q&A carry-forward text below still mentions `T79` or earlier tasks as current, treat it as historical text only.

## 2026-06-12 Captain Supersession (T78 closeout)

- The current unique task for all new worker action is `T79: 论文材料 reopen gate 与 bounded prose 扩写就绪性评审`.
- The authoritative task package is `docs/tasks/Phase2/T79_paper_reopen_gate_and_prose_readiness_review.md`.
- `T78` has been accepted as `PASS`.
- `R35` is closed by `T78`; `R32/R33` remain open.
- `T37` remains `blocked + lowest-priority backlog`.
- If older Q&A carry-forward text below still mentions `T78` or earlier tasks as current, treat it as historical text only.

## 2026-06-12 Captain Supersession (T77 closeout)

- The current unique task for all new worker action is `T78: 论文 note-draft 非结果层校准、statcalib 层级降权与排版 warning 收口`.
- The authoritative task package is `docs/tasks/Phase2/T78_paper_note_alignment_statcalib_hierarchy_and_layout_closeout.md`.
- `T77` has been accepted as `PASS_WITH_WARNINGS`.
...
- `R34` is closed by `T77`; `R35` is the new paper-facing carry-forward risk from `T77`; `R32/R33` remain open.
- `T37` remains `blocked + lowest-priority backlog`.
- If older Q&A carry-forward text below still mentions `T77` or earlier tasks as current, treat it as historical text only.

## 2026-06-12 Captain Supersession (T76 closeout)

- The current unique task for all new worker action is `T77: 论文 note-draft 结果层同步与 T76 traceability hardening`.
- The authoritative task package is `docs/tasks/Phase2/T77_paper_note_results_sync_and_traceability_hardening.md`.
- `T76` has been accepted as `PASS_WITH_WARNINGS`.
...
- `R34` is the new paper-facing carry-forward risk from `T76`; `R32/R33` remain open.
- `T37` remains `blocked + lowest-priority backlog`.
- If older Q&A carry-forward text below still mentions `T76` or earlier tasks as current, treat it as historical text only.
...
- The authoritative task package is `docs/tasks/Phase2/T88_mainline_bounded_manual_finish_and_surface_freeze.md`.
- `T87` has been accepted as `PASS`.
- `T87` opens no new warning-derived risk; its non-blocking notes are accepted as operational reminders only.
...
- 当前暂无 `Linux + FPGA` 硬件宿主，因此 real-board execution 路线仍受 `R33` 约束并保持最低优先级 backlog。
- `T88` 只允许执行已批准的 `MF01-MF05` bounded manual finish、surface freeze、blocked disclaimer 与 frozen-handoff gate，不得借此升级为 submission-ready completion、real-board success、default-env `.tflite` portability closure、full reproducibility closure 或 `statcalib` comparator promotion。
- If older Q&A carry-forward text below still mentions `T87` or earlier tasks as current, treat it as historical text only.
...
- Current authoritative task = `T90: 训练链 clean-CPU 同机 repeated-run 一致性证据包`.
- Current authoritative task package = `docs/tasks/Phase2/T90_training_same_host_repeated_run_consistency_pack.md`.
- `T89` has been accepted as `PASS`; any older `T89 current task` wording above this clarification should be treated as historical carry-forward text only.
...
- `T31` Captain 已接受 review 为 `PASS`；已产出 `docs/evidence_packs/training_reproducibility/training_chain_portable_dependency_lock_plan.md`，但不是 clean-environment rebuild proof。
...
- 当前唯一任务：`T90: 训练链 clean-CPU 同机 repeated-run 一致性证据包`，任务包 `docs/tasks/Phase2/T90_training_same_host_repeated_run_consistency_pack.md`；它只允许在现有 training reproducibility / material-regeneration / clean CPU-only lane 边界内补 helper、tests、3 次 train+eval rerun 与 repeated-run consistency 台账，不得扩到 mainline note 重写、benchmark、HIL、`.tflite` portability、真板成功宣称、submission-ready 完成态、theory branch 大范围改写、sidecar promotion 或 `T37` 真板执行。
...
6. 训练与 recovery benchmark 当前分别依赖哪些最小包集？
   - 当前答案：
     ...
     - 训练链当前单独记录在 `docs/evidence_packs/training_reproducibility/training_chain_bootstrap.md`，推荐解释器为本机 `DLEnv`
     - `.tflite` 路径当前单独记录在 `docs/evidence_packs/deployment_boundary/TFLite_runtime_bootstrap.md`，真实 runtime 依赖尚未满足
...
8. 已跟踪的 `.pyc` / `__pycache__/`、`runs/`、`artifacts/` 何时启动有界 cleanup，并如何拆分“bootstrap 必需”与“历史归档”？
   - 当前答案：
     - `T19` 已产出 `docs/evidence_packs/repo_hygiene/cleanup_tracked_cache_manifest.md`，确认 tracked `.pyc` 共 `116` 个，全部位于 `9` 个 `__pycache__` 目录。
...
9. 下一张继续开发任务包应该优先选哪一类？
   - 当前答案：
     ...
     - 当前唯一任务已切换为 `T90: 训练链 clean-CPU 同机 repeated-run 一致性证据包`，任务包已存在：`docs/tasks/Phase2/T90_training_same_host_repeated_run_consistency_pack.md`；它只允许补 same-host clean CPU-only repeated-run consistency 台账，不直接恢复无界 full-manuscript 扩写，也不触碰 mainline note 正文、benchmark、`.tflite` portability、real-board execution 或 theory 分支大范围改写。
...
12. `T17` 的 review warning 如何处理？
   - 当前答案：
     ...
     - N2 未产出 `requirements-train.txt`：`accepted`，因为任务允许用 `docs/evidence_packs/training_reproducibility/training_chain_bootstrap.md` 收口；训练链可移植性后续单开任务。
13. `T18` 的主要结论是什么？
   - 当前答案：
     - Verdict：`PASS`。
...
16. T20 当前补出的主要结论是什么？
   - 当前答案：
     - `docs/evidence_packs/deployment_boundary/real_board_hil_readiness.md` 已形成。
...
22. T22 当前已经产出了什么？
   - 当前答案：
     - 已新增 `docs/evidence_packs/deployment_boundary/real_board_smoke_execution_plan.md`。
...
41. T30 reviewer warnings 如何处理？
   - 当前答案：
     - N1 gate doc stale non-claim：`accepted`，Captain 已修正 `docs/evidence_packs/statcalib_fr8/statcalib_feasibility_gate.md`。
...
42. T36 是否可以交给 Worker？
   - 当前答案：
     ...
     - T36 已读取既有 `runs/teachrepr*` 结果并产出 `docs/evidence_packs/mechanism_ablation/seed20260429_failure_diagnosis.md` 与只读分析脚本。
...
48. T31 review 如何裁决？
   - 当前答案：
     ...
     - N2 later alignment with `docs/evidence_packs/training_reproducibility/training_chain_bootstrap.md`：`accepted` as future alignment。
...
- `T67` review has been accepted by Captain as `PASS_WITH_WARNINGS`.
- Warning classification:
...
- No new standalone risk item is opened by warning classification for `T67`.
- `R24` remains open, but it is now more specific again: gross teacher-anchor dependence is no longer the dominant concern; generated-only robustness and mixed-row closure are.
- The current unique task is now `T68: FR8 statcalib generated-only robustness bounded benchmark`, task package `docs/tasks/Phase2/T68_fr8_statcalib_generated_only_robustness_bounded_benchmark.md`.
- `T68` exists to answer whether a predeclared non-`ukf` statcalib candidate can beat both frozen anchors in all four locked scenarios while keeping `statcalib_status = generated` in all four rows, without rewriting `T24` or upgrading the evidence beyond mock-backed software-HIL scope.

## 2026-06-08 Captain Update (T68 risk supersession)

- `T68` has been accepted as `PASS_WITH_WARNINGS`.
- Warning classification:
...
- `R24` remains open, but it is now narrower than before:
  1. the repo no longer needs to ask whether any full generated-only winner exists inside the predeclared grid
  2. the repo still needs to avoid overclaiming a unique final threshold or a uniformly clean full grid
