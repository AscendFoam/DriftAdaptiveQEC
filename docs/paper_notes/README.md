...
- `Contract_Centric_Regime_Aware_GKP_note_draft.tex`：2026-07-20 同步到 T7.2.1 的正式论文式 Route-A note。标题仍为 restricted pre-board contract paper；摘要只保留 T7.1.1 允许的合同集成、locked-EWMA smooth 与确定性预板架构三条主张。T7.2.1 将正文改成 6 段 Introduction 与 5 个机制分组 Related Work，按 static/CI-ML/drift/NN-RL-NMF/AQEC/CPD/FPGA task signature 分 lane，不隐藏 static/Window、V5 early-stop、NMF/learned/FPGA eligibility 和 board-null。
