...
the accepted result layer.  In a submission-facing assembly, only the locked
benchmark layer and its conservative interpretation belong to the main text;
appendix, supplement, and excluded surfaces remain routed separately.
...
frozen reference protocol, the hybrid residual branch is the winner in all
four frozen scenarios, with UKF as the runner-up throughout.  This remains
the authoritative mainline result layer of the current note.  It supports a
...
hardware validation.  At this point, the remaining manuscript-side work is
bounded submission-facing assembly under these existing layers, not promotion
of a new evidence layer.  Any such assembly should therefore keep the frozen
ranking and conservative mechanism reading in the main text, move supporting
tables to appendix or supplement by explicit route, and leave blocked hardware
or portability surfaces outside the package claims.
...
manuscript side, the current repository can honestly support a
simulation/material-first route: frozen-set main results in the main text,
bounded ablation plus training/material and isolated true runtime tables in the
appendix, and a calibration-extension supplement plus real-board
gate/provenance materials in the supplement.  The remaining manuscript work is
to assemble these already translated layers into a bounded submission-facing
package without upgrading any claim.  On the execution side, broader drift
families, stronger oracle or
...
honest next route at this stage is therefore bounded submission-facing
assembly around an explicitly layered evidence stack, not deployment closure,
submission-ready completion, or hardware-ready finalization.  The assembly may
package main-text results, appendix support, and supplement-only gated
surfaces together, but it must continue to exclude blocked hardware,
portability, and promoted-comparator claims.
