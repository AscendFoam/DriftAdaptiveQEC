...
four-scenario mock-backed software-HIL protocol anchored by `T24'.  Inside
that frozen set, the teacher-anchored hybrid residual branch is the winner in
all four scenarios and UKF is the runner-up throughout.  This is the mainline
ranking result; it is not an expanded benchmark claim, not a
deployment-closure result, and not a real-board result.
...
The same affine runtime contract can host a simple non-neural
histogram-driven calibration rule.  Under the bounded FR8 lane, that
supplement-side `statcalib' path can be strong relative to the frozen anchors,
which supports the calibration-contract story at the project level.  However,
the accepted closure remains extension-lane only, explicitly no-promotion, and
without a unique clean threshold.
...
read-only real-board gate/regeneration/provenance pack with current-host
`NO\_GO'.  These are useful supporting boundaries for the manuscript, but they
remain layered evidence rather than coequal benchmark results or completed
deployment claims.
...
This section records the accepted mainline result layer, its bounded support
layers, and the follow-up routes that remain explicitly outside the accepted
result layer.
...
\subsection{Bounded follow-up lanes outside the accepted result layer}

The main benchmark and paper-material lane should remain separated from
parallel extension experiments.  This subsection is therefore a boundary
register rather than an additional Results pillar: it records follow-up lanes
that may be explored in parallel with the mainline, but whose outputs remain
sidecar evidence until a later promotion gate decides otherwise.  In
particular, these routes must not rewrite the frozen four-scenario anchor,
promote the statistical-calibration lane into a mature comparator, or turn
software-HIL evidence into \texttt{.tflite} or board-level validation.
...
preserved artifacts.  Supplement-only materials can carry the FR8
\texttt{statcalib} closure pack with its no-promotion / no-unique-threshold
...
is not a weakness of the write-up; it is the mechanism that prevents supporting
evidence from being misread as new ranking evidence or completed hardware
validation.  At this point, the remaining manuscript-side work is mainly
reader-facing condensation and route cleanup, not promotion of a new evidence
layer.
...
appendix, and FR8 extension-lane plus real-board gate/provenance materials in
the supplement.  The remaining writing work is to translate these internal
layers into a final reader-facing polish pass without upgrading any claim.  On
the execution side, broader drift families, stronger oracle or
...
honest closeout route at this stage is therefore bounded final polish around an
explicitly layered evidence stack, not deployment closure or hardware-ready
finalization.
