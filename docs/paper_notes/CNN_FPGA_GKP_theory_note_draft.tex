...
scenarios, with UKF as the runner-up throughout.  The newer Phase A
formal-repeat source data strengthen two scenario rows: for
...
and hardware gates remain open.  Bounded ablation and multi-seed materials
...
story remains non-causal.  A separately labeled
...
clean threshold.  Controlled oracle-affine, wrapped-Gaussian, holdout-stress,
operation-count, and fixed-point checks add theory-facing diagnostics, not a
...
a real-board result.  The later Phase A formal-repeat source data add
...
complete the all-scenario repeat-expanded gate.
...
from \(1.75\%\) to \(2.89\%\) against UKF in the four frozen scenarios
(Table~\ref{tab:paired-margin-note}).  The average relative reduction across
the tested set is \(2.32\%\).  Because the frozen anchor has only two repeats
per scenario, these margins remain descriptive source-data readouts.  They are
not confidence intervals, \(p\)-values, standard errors, or a distributional
robustness claim.
...
\begin{tabular}{lrrrr}
\toprule
Scenario & UKF & Hybrid-\(b\) & Mean \(\Delta\) & Rel. red. \\
\midrule
\texttt{static\_bias\_theta} & 0.825370 & 0.810902 & 0.014469 & 1.75\% \\
\texttt{linear\_ramp} & 0.811201 & 0.787755 & 0.023446 & 2.89\% \\
\texttt{step\_sigma\_theta} & 0.811548 & 0.788800 & 0.022748 & 2.80\% \\
\texttt{periodic\_drift} & 0.821558 & 0.806392 & 0.015166 & 1.85\% \\
\bottomrule
\end{tabular}
\caption{Descriptive UKF-minus-hybrid margins in the frozen benchmark.
\(\Delta\) is UKF \(\texttt{final\_ler}\) minus Hybrid-\(b\)
\(\texttt{final\_ler}\).  Positive values indicate lower hybrid error.  The
rows summarize the available paired source data only; they are not inferential
statistics or expanded-benchmark evidence.}
\label{tab:paired-margin-note}
\end{table}

\subsection{Formal Phase A paired-interval check}

The submission draft adds a repeat-expanded Phase A source-data layer for the
...
(Table~\ref{tab:phase-a-interval-note}).  This supports a scenario-level
...
\begin{tabular}{lrrr}
\toprule
Scenario & Paired repeats & Mean delta & 95\% intervals \\
\midrule
\texttt{static\_bias\_theta} & 12 & 0.015563 &
\([0.014778,0.016348]\); boot. \([0.014933,0.016256]\) \\
\texttt{linear\_ramp} & 12 & 0.022417 &
\([0.021215,0.023619]\); boot. \([0.021405,0.023440]\) \\
\bottomrule
\end{tabular}
\caption{Formal Phase A scenario-level UKF-minus-hybrid interval checks.  The
rows summarize the completed formal-length paired repeats for two scenarios
only; they do not complete the all-scenario gate.}
\label{tab:phase-a-interval-note}
\end{table}
...
of an assumed latency footnote.
...
\begin{tabular}{lrrrr}
\toprule
Holdout stress family & Fixed & Lagged & Oracle & Wrapped mean \\
\midrule
\texttt{random\_walk\_drift} & 0.078869 & 0.073867 & \textbf{0.072685} & 0.078740 \\
\texttt{burst\_reset\_drift} & 0.066865 & 0.068622 & \textbf{0.062144} & 0.065936 \\
\texttt{faster\_than\_window} & 0.068354 & 0.068890 & \textbf{0.063745} & 0.067485 \\
\bottomrule
\end{tabular}
\caption{Controlled known-state holdout drift stress diagnostic.  Entries are
residual MSE across 384 sequences of 512 steps; lower is better.  The lagged
row is a known-state slow-commit reference, not the trained residual branch.
The table is a non-hardware stress test, not holdout robustness proof.}
...
that diagnostic.  The known-state oracle affine row improves on fixed affine
in all four controlled states, with the largest headroom after the step
change.  The wrapped posterior mean is slightly better only in the static
state, and the MAP branch rule is weaker in the ramp, step and periodic
states.  The result strengthens the engineering rationale for calibrated
affine updates, but it does not replace a formal known-noise nearest-lattice or
sequence-level wrapped-decoder benchmark.
...
\begin{tabular}{lrrrr}
\toprule
Controlled state & Fixed & Oracle & Wrapped mean & Wrapped MAP \\
\midrule
\texttt{static\_bias\_theta} & 0.049199 & 0.048095 & \textbf{0.047955} & 0.048203 \\
\texttt{linear\_ramp\_midpoint} & 0.061417 & \textbf{0.060251} & 0.062341 & 0.063105 \\
\texttt{step\_after\_jump} & 0.108216 & \textbf{0.092541} & 0.105815 & 0.110339 \\
\texttt{periodic\_high\_phase} & 0.081757 & \textbf{0.076028} & 0.082349 & 0.084002 \\
\bottomrule
\end{tabular}
\caption{Controlled one-step local-Gaussian residual-MSE diagnostic.  Lower is
better.  The omitted direct nearest-syndrome row is substantially weaker in
the same source table because it applies the noisy wrapped syndrome as a hard
correction.  This is a sanity check for estimator and decoder limits, not a
formal software-HIL or hardware benchmark.}
...
\midrule
Affine fast path & 4 & 4 & 0 & six stored state scalars \\
Wrapped MAP, \(3\times3\) branches & 49 & 40 & 0 & larger branch search \\
Wrapped posterior mean, \(3\times3\) & 99 & 98 & 18 & softmax-like posterior \\
Q4.20 fixed-point parity & -- & -- & -- & max diff \(1.6\times10^{-6}\), no crossing delta \\
\bottomrule
\end{tabular}
...
software emulation only.  The table supports the bounded datapath rationale,
not measured FPGA timing, resources, power, or source-vs-board agreement.}
...
hardware validation.  The submission-draft synchronization adds theory-facing
diagnostics and two completed Phase A scenario-interval rows, but it does not
change that layered route.  The compact note should still keep the ranking and
conservative mechanism reading in the main text, place controlled diagnostics,
training/material provenance and runtime-boundary tables in supporting
material, retain the calibration-extension and real-board gate materials as
bounded side surfaces, and leave blocked hardware or portability claims outside
the package.
...
manuscript side, this note now carries a compact synchronization of the
submission draft's theory and Phase A evidence: two completed formal scenario
rows support positive UKF-minus-hybrid interval statements, and controlled
oracle, holdout, operation-count, fixed-point and runtime-discipline checks
make the affine contract more auditable.  Those additions remain supporting
evidence.  On the execution side, completion of all formal scenarios, stronger
sequence-level baselines, default-environment \texttt{.tflite} portability, and
any board-level timing claim still require separate bounded tasks.  In
...
honest handoff is therefore an evidence-synchronized note with explicit
blocked-surface disclaimers, not deployment closure, submission-ready
completion, or hardware-ready finalization.
