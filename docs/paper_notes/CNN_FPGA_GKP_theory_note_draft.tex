...
\title{Drift-Adaptive Affine GKP Decoding with Teacher-Anchored Residual Calibration}
\author{}
\date{}
...
labeled supplement-side statistical-calibration extension lane further suggests that simple
...
fixed-point affine displacement.  In the current note, only the locked
software-HIL benchmark is treated as the main result layer; deployment-facing
material and the later statistical-calibration lane remain bounded follow-up
evidence rather than coequal main results.
...
exported-model runtime claim; and read-only real-board gate, regeneration, and
provenance checks preserve a current-host \texttt{NO\_GO} hardware boundary.
These layers do not yet establish board-level latency, FPGA
...
separately labeled supplement-side statistical-calibration extension lane shows
that simple non-neural estimators can also be strong under the same runtime
contract, but the accepted closure remains no-promotion and no unique clean
threshold.
