...
### 11.6 参考：NVIDIA 论文启发与可吸收的方法论

参考论文：[relative_papers/Fast and accurate AI-based pre-decoders for surface codes.md](relative_papers/Fast%20and%20accurate%20AI-based%20pre-decoders%20for%20surface%20codes.md)

**总体判断**：该论文对当前项目有明确启发，但更适合作为方法论参考而不是直接照搬网络结构。原因是论文主问题是 surface code + global decoder 的预解码，而当前项目是 GKP + FPGA 双回路的闭环控制。

**可直接借鉴的 5 条方法论**：

| # | 方法论 | 当前项目对应 | 状态 |
|---|--------|-------------|------|
| 1 | 训练目标要尽量贴近下游真正关心的量，而不是停留在中间参数拟合 | `residual-b`（直接修正控制偏置）而非绝对参数回归 | 已采纳 |
| 2 | 全局统计量更适合走低维聚合支路，而不是粗暴广播成整张平面通道 | `Gated v5` 的 scalar_branch + gated 注入 | 已采纳 |
| 3 | 稳定性最好在模型内表达一部分，而不是完全依赖外部 EMA | 待实现（paper-inspired 分支的 stat summary + in-model aggregation） | 待落地 |
| 4 | 可识别性优先于模型容量：不能从当前统计量稳定识别出的量，不应强行让 CNN 去学 | 早期 theta 学不稳 → 改为各向异性数据；teacher params 高冗余 → 只保留 4 个关键标量 | 已采纳 |
| 5 | learned module 的角色要清楚：是"校准器/残差补偿器"，不是"完全替代解码器" | `Hybrid Residual-B` 的核心叙事 | 已采纳 |

**不能直接照搬的地方**：
1. 论文的下游是 PyMatching/MWPM 全局图解码器，当前项目下游是 `(K, b)` 线性解码器
2. 论文偏 GPU 高吞吐批量并行，当前项目需满足 ARM/FPGA 双回路部署时延约束
3. 论文中更大的 2D/3D CNN 结构不能直接作为当前默认主线（会压过已验证的轻量部署路线）

**论文对当前项目的叙事价值**：
- 它帮助确认：当前从"绝对参数回归"走到"teacher-guided residual-b"方向本身是对的
- 它帮助解释：问题不在于"整包 teacher 信息有害"，而在于"整包广播式编码不适合闭环任务"
- 后续论文中可将当前工作表述为：`a hardware-aware learned calibration / residual correction module for real-time decoder adaptation under drift`

---

### 11.6 跨任务需保持的工程一致性原则

以下原则是经过多轮实验反复验证后形成的隐性约束，Captain 在分配任何新任务时必须确保 Worker 不违反：

**原则 1：ParamMapper 语义冻结**

`cnn_fpga/decoder/param_mapper.py` 中 `(σ, μ_q, μ_p, θ) → (K, b)` 的映射公式、特征值裁剪、偏置项表达和参数平滑规则已全部冻结。任何训练、benchmark 或 HIL 运行中的 ParamMapper 语义必须保持一致。若确需修改（极少情况），必须在独立对照实验中单独评估，不能混入其他改动。

冻结的核心公式：
- `K_raw = C·(C + R_meas)⁻¹`（Wiener/Kalman 形式）
- `K_target = U·diag(clip(λ_raw, [g_min, g_max])·gain_scale)·Uᵀ`
- `b_target = α·(I - K_target)·μ`（非 `-α·μ`）
- `K_next = (1-β)·K_prev + β·K_target`（EMA 平滑）
- `b_next = (1-β)·b_prev + β·b_target`

**原则 2：benchmark 口径不可隐式修改**

"benchmark 口径"包括：
- 场景定义（噪声参数 + 时间 envelope）
- Baseline 集合（哪些模式参与对比）
- 评价指标（LER、overflow_rate 及其三个子项）
- Seed 策略（paired seeds / independent seeds）
- repeat 数、窗口数、commit 数

任何改动都必须在实验记录中显式声明，不得在"调参"过程中顺便修改。

**原则 3：离线训练与 formal HIL 必须分开评估**

不能因为离线训练指标变好就直接断定 formal HIL 更优。任何模型变体必须经过 formal HIL paired benchmark（至少 2 seeds × 4 scenarios）才算完成闭环验证。

**原则 4：边界标签不可升级**

第 7 节的 6 种边界标签是 T3 固定下来的。只有在新证据覆盖当前 placeholder/stub 状态时才能升级。例如：只在有真实板卡设备节点 + RTL 地址表 + 成功联调证据时，才能把 `placeholder_real_board_backend` 升级。

**原则 5：文档状态与代码状态必须同步**

每次代码改动如果改变了以下任一事实，必须同步更新对应文档：
- 默认参数变了 → 更新阶段结论文档
- 模型结构变了 → 更新本文件和配置说明
- benchmark 口径变了 → 更新 P4 bootstrap 文档
- 环境/依赖变了 → 更新第 6 节

### 11.7 参考：NVIDIA 论文启发与可吸收的方法论
...
### 14.4 Captain AI 常见陷阱与注意事项

以下是历史开发中已被反复踩过、新 Captain 不应再重复的问题：

1. **不要把 `board_backend.py` 写成真板完成**
   - 文件顶层直接标注为 `Placeholder real-board backend`
   - `schedule_commit()` 返回大量 `None`，`step()` 返回空事件
   - 无论 benchmark 结果多稳定，都不能声称"真板 HIL 已通过"

2. **不要混淆真实 .tflite 和 stub .tflite**
   - `export.py` 支持真导出 + stub 回退
   - `inference_service.py` 区分 `tflite_service` 和 `tflite_stub_service`
   - 当前 Windows 环境没有 `.venvs/tf311`，.tflite 独立验收不可用
   - 配置里写了 `backend=tflite` 不等于"真实 TFLite 部署已完成"

3. **不要把离线训练改善等同于 formal HIL 改善**
   - No TeacherParams 离线 3/3 seed 稳定更好 → formal HIL 随 seed 翻转
   - Gated v8 离线部分指标改善 → formal HIL 整体不够稳
   - 永远要求 formal HIL paired benchmark 作为最终判断依据

4. **不要继续微调 gated 超参**
   - v8→v9 已经证明 gate/clip/scale 微调边际收益极低，甚至退化
   - "更激进 → 翻车，更保守 → 收益消失" 表明瓶颈不在超参
   - 应转向结构升级（paper-inspired）或机理诊断

5. **不要把 PB Bound/PB ST 写成论文主线**
   - 它们只在特定动态场景下有优势，不是跨场景统一更优
   - 定位只能是"场景特定辅助机制分析支线"

6. **不要把 single-scenario/repeats=1 的 recovery smoke 写成正式 benchmark**
   - Recovery smoke 只证明"链路可运行"
   - 正式结论需要多场景 + 多 repeats + 多 seed

7. **不要在没有真板证据的情况下扩展 board backend 的语义**
   - `board_backend.py` 是 placeholder，不应对其进行"看起来更像真板"的扩写
   - 除非有真实的设备节点和 RTL 地址表，否则扩写只会制造更多假完成

8. **不要同时改 ParamMapper 和模型训练**
   - ParamMapper 公式已冻结（第 2.6 节）
   - 若确实需要改 ParamMapper，必须把它作为独立对照实验，不能混入同一轮 benchmark

### 14.5 项目中的特殊约定与术语

| 术语 | 含义 | 备注 |
|------|------|------|
| `fixed_baseline` | 始终使用固定参数，不自适应 | P2/P3 工程下界 |
| `oracle_delayed` | 使用延迟一窗的 target 参数 | P2 公平上界参考 |
| `fixed_baseline_mock` | 同 `fixed_baseline`，但在 mock backend 下 | P3 软件 HIL 统一参照 |
| `mock` (backend) | `cnn_fpga/hwio/mock_fpga.py` | event-driven FPGA 行为仿真 |
| `board` (backend) | `cnn_fpga/hwio/board_backend.py` | placeholder 真板后端 |
| `inproc` | 进程内 NumPy 推理 | 当前 recovery smoke 的 inference 方式 |
| `artifact_npz` | 直接读取 `.npz` 模型参数文件做推理 | 当前 recovery smoke 的 artifact backend |
| `tflite_service` | 真实 TFLite 解释器推理 | 需 TensorFlow/TFLite 运行环境 |
| `tflite_stub_service` | 从 `.tflite.json` stub manifest 读取参数推理 | 回退路径，不等同于真实 TFLite |
| `paired seeds` | 不同 mode 使用相同的噪声/场景 seed | 确保对比公平 |

### 14.6 端到端运行时数据流详解

以下是从 syndrome 产生到参数更新的完整数据流，帮助 Captain 理解各模块间的实际调用关系：

**第一阶段：快回路（每个 fast cycle, 目标 ≤5μs）**

```
1. physics/syndrome_measurement.py
   └─ RealisticSyndromeMeasurement.measure(true_displacement)
      → noisy syndrome (s_q, s_p) [float32×2]

2. cnn_fpga/decoder/linear_runtime.py
   └─ LinearRuntime.decode(s_q, s_p)
      → fixed-point Q4.20: s_q_fxp, s_p_fxp
      → Δ = K @ [s_q_fxp; s_p_fxp] + b  [fixed-point multiply-accumulate]
      → correction (Δ_q, Δ_p) [clamped to Q4.20 range]

3. cnn_fpga/runtime/fast_loop_emulator.py
   └─ FastLoopEmulator.fast_cycle()
      → histogram 累积: clip(s_q, s_p) → bin index → increment bin counter
      → overflow 统计: histogram_input_saturation / correction_saturation
```

**第二阶段：窗口就绪检测与慢回路触发**

```
4. cnn_fpga/runtime/scheduler.py
   └─ Scheduler.check_window()
      → window_ready = (fast_cycle_count % W == 0)
      → 触发 slow_update 事件

5. cnn_fpga/runtime/slow_loop_runtime.py
   └─ SlowLoopRuntime.update(histogram, epoch_id)

      a) 取 histogram 窗口 + 归一化
      b) cnn_fpga/runtime/feature_builder.py
         └─ FeatureBuilder.build(histogram_window, teacher_state)
            → 多窗口 histogram tensor [context_windows × 32 × 32]
            → histogram deltas (窗口间差分)
            → teacher prediction / params / deltas (scalar or broadcast)
            → 输出: feature tensor + scalar_branch tensor

      c) 推理（根据 mode）:
         - inproc + artifact_npz: 直接加载 .npz 权重的 NumPy forward pass
         - tflite + tflite_service: subprocess 调用真实 TFLite 解释器
         - tflite + tflite_stub_service: 读取 .tflite.json stub manifest

      d) CNN 输出: delta_b_q, delta_b_p [float32×2]
         → b_next = teacher_b + delta_b (或 EMA 版本)

      e) cnn_fpga/decoder/param_mapper.py
         └─ ParamMapper.map(sigma_hat, mu_q_hat, mu_p_hat, theta_hat)
            → 误差协方差 C_principal → 实验室系 C
            → K = C·(C+R)⁻¹ (特征值裁剪)
            → b = α·(I-K)·μ
            → EMA 平滑
            → 输出: K (2×2), b (2×1)
...
```
6. cnn_fpga/runtime/param_bank.py
   └─ ParamBank.stage(K_new, b_new)
      → 写入非活跃 bank (bank_B)
      → stage_epoch = current_epoch

7. cnn_fpga/hwio/mock_fpga.py (或 board_backend.py)
   └─ schedule_commit(epoch)
      → 在周期边界检测 commit_epoch
      → 原子切换: active_bank = staged_bank
      → commit_ack 事件产生
      → 回显确认

8. cnn_fpga/decoder/linear_runtime.py
   └─ 下一 fast cycle 自动使用新 bank 的 K, b
```

**延迟注入点**（`cnn_fpga/runtime/latency_injector.py`）：
- DMA 读出延迟 (5~20μs)
- 预处理延迟 (20~100μs)
- CNN 推理延迟 (0.1~2ms)
- 参数写回延迟 (10~100μs)
...
**日志记录**（每次 run）：
- `hil_events.json`: 每个事件的 timestamp + epoch_id + event_type
- `hil_summary.json`: 汇总 LER / overflow_rate / violation_rates / commits / backend / artifact info
- `comparison.csv` (P4): 多 mode 多场景的最终指标对比

### 14.7 项目关键时间线与版本演进

| 时间 | 里程碑 | 关键事件 |
|------|--------|----------|
| 2026-03-17 | P0 完成 | full_qec vs simplified LER gap 确认 |
| 2026-03-19 | P1/P2 完成 | static_theta_v2 模型训练 + P2 公平基线对比 |
| 2026-03-28 | P3 软件 HIL 打通 | mock + float/int8/real_board 四 mode 对比 |
| 2026-03-29–31 | P3 overflow/输入范围 | 三轮参数调优 + 输入范围调参 + 新默认值回写 |
| 2026-04-01 | P4 首轮 | 方案切换：AbsReg → teacher + residual-b。首轮 formal benchmark |
| 2026-04-02 | P4 长配置复验 | 输入范围再提升 + 更长配置。Hybrid 优势扩大 |
| 2026-04-03 | P4 强 baseline | UKF 修正 + RLS。UKF 成为最强经典 baseline |
| 2026-04-04–05 | P4 features ablation | histogram delta 确认为关键通道；No TeacherParams 异常信号 |
| 2026-04-05–17 | No TeacherParams 路线 | 离线训练优势稳定；formal HIL 随 seed 翻转 → 路线终止 |
| 2026-04-26 | Physical Bridge 支线 | PB Bound/ST 确认为场景特定辅助，不进入主线 |
| 2026-04-27–29 | Gated v5 确立 | 3 seed/4 scenario paired benchmark。当前最强 candidate |
| 2026-04-28–05-02 | Gated v8/v9 | 过冲控制尝试。v9 明显退化。gate 微调路线终止 |
| 2026-05-02 | Legacy handoff | 交接文档撰写：建议停止微调，转向机理诊断/结构升级 |
| 2026-05-05 | 恢复期启动 | D-2026-05-05-01：仓库进入 Repair |
| 2026-05-05–06 | Phase 0 (T0–T4) | 治理恢复 + 环境确认 + mock/stub 边界固定 |
