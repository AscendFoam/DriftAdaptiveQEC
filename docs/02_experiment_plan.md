...
| 优先层 | 候选任务 | 主要输出 | 验证 |
| --- | --- | --- | --- |
| Current | `T77` note-draft results sync 与 `T76` traceability hardening | paper note 结果层同步、preview-source schema 清理、逐图 stable-ID 显式绑定、可选 note 编译检查 | 文档一致性检查 + note 编译检查（如可用） + reviewer |
| Immediate-next | paper draft reopen gate（条件式） | 判断 `T77` 之后的材料栈是否足够支持受控 prose reopen；若不足则定义下一张补缺任务 | captain / reviewer gate |
...
| P3 | paper draft reopen after gate | outline、abstract、figures、related work 的受控 prose reopen | only after `T77` and supporting materials are judged sufficient |
