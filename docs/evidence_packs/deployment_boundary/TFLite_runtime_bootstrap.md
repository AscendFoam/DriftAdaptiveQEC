...
本文件只服务 `.tflite` export/runtime 路径的独立接力，不替代 `docs/evidence_packs/training_reproducibility/training_chain_bootstrap.md`，也不替代 `requirements-recovery.txt`。
...
2. `docs/evidence_packs/training_reproducibility/training_chain_bootstrap.md`
   - 只覆盖训练链入口与环境边界
3. `docs/evidence_packs/deployment_boundary/TFLite_runtime_bootstrap.md`
