...
这部分是后续最值得直接复用的经验。结论很简单：`glm-ocr:latest` 不稳定时，不要只对同一张图重复重试，要改变输入形态。

### 11.1 先缩放，再送模

- 整页长图直接送给模型时，最容易触发 `HTTP 500`
- 实测里，`384` 和 `256` 这两个尺度并不总是稳定
- 更稳的起点通常是 `192`、`128`、`96`
...
- 有些页在 `RGB` 下直接 500
- 同一页换成灰度或二值后就能过
- 也有少数页只在 `RGB` 下稳定，灰度反而失败

所以不要固定一种预处理，应该轮询。

### 11.3 长页优先切片

对密集版式、长公式页、表格页，整页失败概率明显更高。
...
如果目标是“最后所有页面都能交付”，不要把失败结果停留在 `[OCR_FAILED: ...]`。
...
- 失败页要单独缓存，不要重跑整本书
- 成功页不要重复 OCR
- 先做小样本对照，确认哪种尺度和预处理最稳
- 如果本地 Ollama 持续 `500`，优先切换输入形态，不要无脑重复请求

### 11.6 经验总结

一句话：

> `glm-ocr` 不是“坏页识别不了”，而是“输入形态不合适时会不稳定”。
> 先缩放、再换预处理、再切片，最后再用文本层兜底，才适合批量跑整库。
