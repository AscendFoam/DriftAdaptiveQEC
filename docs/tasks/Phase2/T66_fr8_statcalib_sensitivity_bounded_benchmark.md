...
- Exact benchmark commands executed:
...
- Verification command outputs:
  - `C:\ProgramData\anaconda3\python.exe -m py_compile cnn_fpga/benchmark/summarize_statcalib_sensitivity.py`
    - passed
  - `C:\ProgramData\anaconda3\python.exe -m unittest tests.test_statcalib_sensitivity_summary`
    - `Ran 5 tests`, `OK`
  - `C:\ProgramData\anaconda3\python.exe -m cnn_fpga.benchmark.summarize_statcalib_sensitivity --run-dir runs/p4_benchmark/T66_statcalib_sensitivity_20260529_210906`
    - best aggregate variant by mean LER: `statcalib_high_threshold`
    - wrote `statcalib_sensitivity_summary/summary.json`
  - provenance/integrity
    - launch commit = finish commit = `summary.json git_commit` = `ad981bb`
    - exactly one T66 run root exists
    - no historical `T24/T64/T61/T62` run-root last-write time moved during T66
    - `missing_runs=[]`
    - `comparison_rows=28`
    - `raw_rows=56`
...
- Whether the best statcalib variant beats `ukf`:
  - Yes. The best aggregate variant by mean LER, `statcalib_high_threshold`, beats `ukf` in all four locked scenarios.

- Whether the best statcalib variant beats `hybrid_residual_b`:
  - Yes. The best aggregate variant by mean LER, `statcalib_high_threshold`, beats `hybrid_residual_b` in all four locked scenarios.

- Remaining risks or interpretation limits:
  - still mock-backed software-HIL only
  - not a rewrite of `T24`
  - not `.tflite`
  - not real-board
  - the first foreground launch hit shell timeout, so the same full-matrix command was relaunched in the background under the same run root
  - `progress.jsonl` therefore keeps one duplicate `running` record for `static_bias_theta/statcalib_default/repeat_01`
  - `static_bias_theta / statcalib_high_threshold` is numerically best in that scenario but its aggregated statcalib status is `mixed`
