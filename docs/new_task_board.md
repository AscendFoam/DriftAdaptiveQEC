...
当前推荐任务：`T7.3.2`（T7.2.6 已建立独立于 51 页历史快照的 Phase6D canonical manuscript：multimode software lane 正面报告 strongest-baseline `0% [0%,0%]` NO-GO、pilot/formal/scaling 未访问与 frozen-benchmark SOTA 未建立；exact single-mode RTL lane 报告 6-cycle/II=1、17/17 formal、21/21 mutants、百万周期全 148-byte vector 零 mismatch、三 seed whole-harness P&R estimate，并保持全部 board 字段 null；CNN/student 仅 Dropped ablation。10 claims、57-row lossless Source Data、27/27 gates/mutations、12 页 TeX/BibTeX 零 warning/overfull/undefined/fatal 和 12/12 页视觉 QA 通过。T7.3.1 仍 Blocked，下一项按可执行顺序回答 CNN-centric/模拟器过拟合审稿问题）。
...
| T7.3.2 | In Progress | 回答“CNN 是否只是过拟合模拟器，项目是否仍是 CNN-centric？” | 明确 CNN/teacher/student 只蒸馏 posterior/LLR/coset/action，是 matched-budget replaceable approximation；T6.26.2 不过门就只进消融，不能改变 classical algorithm 或 RTL verdict。 | `T6.26.1--T6.26.2`, `experiment_plan.md §19` |
