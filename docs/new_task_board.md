...
当前推荐任务：`T7.2.2`（T7.2.1 已完成 evidence-bounded Introduction / Related Work：主 note 新增 6 段 Introduction 与 5 个 mechanism-grouped Related Work 小节，30 个 citation keys 全部解析，14 条 claim/citation/project-source 行覆盖 static GKP、CI/ML、drift/calibration、Direct NN/RL/NMF、AQEC、CPD 与 FPGA 六条 task-signature lane。18/18 gates 和定向语义 mutations、8 项专项测试通过；38 页 TeX Live/BibTeX 编译无 overfull/未定义引用/LaTeX warning，38/38 页视觉 QA 通过。正文显式保留 static/Window 反例、tail 仅 non-inferiority、V5 early stop、NMF exact ineligible、learned/FPGA same-task comparator=0 与板测 null；下一步按实际 V5 Dropped 状态起草可执行、truth/observed 权限分层的 Methods）。
...
| T7.2.2 | In Progress | 起草 Methods。 | 覆盖 protocol-aligned simulator、unified contract、四分割预注册、multiscale wrapped features、IMM/BOCPD、activation prediction、posterior integration、risk calibration、typed bank policy、matched baselines、统计、fixed-point/formal/CXXRTL/P&R；逐项标出 online observed-only 与 offline truth-only scoring，真实板卡方法保持 future work。V5 已 early-stop，故未执行的 V5 method/compiler/formal/RTL 只能写成预注册停止分支，不得伪装为 implemented method。 | `T6.10--T6.15` |
