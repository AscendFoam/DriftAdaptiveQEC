...
当前推荐任务：`T7.2.3`（T7.2.2 已完成 evidence-state-bounded Methods：41 页主 note 的 Methods 现覆盖 protocol-aligned simulator、observed/truth 物理分离、统一 packet/LUT/budget/bank 合同、V4 HMM+Window/EWMA、matched baselines、V4 calibration/pilot/formal 统计、V5 causal-headroom 诊断、integer/CXXRTL/P\&R estimate 与真板 future procedure。18-row 方法矩阵把 `IMPLEMENTED_EVALUATED`、`DIAGNOSTIC_ONLY_EXECUTED`、`CONDITIONALLY_REGISTERED_STOPPED`、`FUTURE_PHYSICAL_WORK_BLOCKED` 四态逐项冻结；18/18 gates/mutations、9 focused/50 adjacent tests、41/41 页视觉 QA 通过。V5 四分割 manifest、multiscale/IMM/BOCPD/activation prediction、posterior compiler、LER/CVaR gate、typed V5 bank、formal/CXXRTL/P\&R 均明确为未实例化/未运行，42 个板测字段仍 null；下一步只起草实际存在的 V4/diagnostic/secondary Results）。
...
| T7.2.3 | In Progress | 起草 Results。 | 先完整报告 V4 的 Window/static/tail 负结果、fallback/false-update 代价与 causal-headroom 诊断，再报告 V5 在 entry gate 后 early-stop、因此不存在 V5 untouched LER/tail、quantized retention、formal safety、long CXXRTL 或 P\&R 结果；不得以 V4 数值填补。Phase 6C 只有 `OFFICIAL_CODE_REPRODUCTION` 或 `PROJECT_NATIVE_MATCHED` 结果可进入明确标注的 secondary Results，`LITERATURE_ONLY` 仅进 Related Work/Supplement。失败 family/restart/attempt 不隐藏，measured hardware 不生成结果段。 | `T6.7--T6.15`, `T6.19.3` |
