...
当前推荐任务：`T6.20.3`（T6.20.2 已将 multimode software LER 与 single-mode production RTL 冻结为两个不可互相补门的 primary lane，并把 CNN/student 固定为依赖性近似扩展；机器合同含 13-field task signature、5 条 claim、4 个 integration interfaces、9 条 forbidden transfer、10 个 live evidence bindings、31-row Source Data 与 16/16 semantic mutations。下一步在访问任何新 pilot/formal outcome 前冻结与 T6.18.3 全互斥的四分割、统计功效和 immutable manifest）。
