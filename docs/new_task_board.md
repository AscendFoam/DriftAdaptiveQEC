...
当前推荐任务：`T7.1.1`（T6.19.3 已完成六 lane 只读 comparison atlas：206 个 evidence cell 中含 18 个外部 FPGA implementation×9 个 latency/clock/cycle/II/resource/power 字段，逐 cell 绑定 source report、raw Source Data、冻结 config 与 SHA-256；N/A/null/failed/negative 均显式保留。Noh CNOT counts/Wilson CI、official CPD threshold、multimode raw counts/worst-window、AQEC 20k cluster bootstrap、FPGA 6-cycle/II=1 换算及 external comparator count 全部独立重算。24/24 gates、24/24逐门 mutations、162 项 Phase 6C 联合回归通过，Python 图完成 SVG/PDF/TIFF/PNG 与视觉 QA；verdict=`PASS_AUX_COMPARISON_INTEGRITY`。T6.15.5 仍为 early-headroom NO-GO，T6.9.2 仍 Blocked，global/cross-lane/faster/SOTA claim 继续关闭。下一步冻结 claim-evidence-boundary matrix）。
