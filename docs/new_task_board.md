...
当前推荐任务：`T7.1.2`（T7.1.1 已冻结 29 条 claim-evidence-boundary 原子主张：完整覆盖 V4 11 条、V5 early-stop 10 条和 Phase 6C 六 lane 8 条；45 个 report/Source Data/code artifact 均 live hash-bound。19/19 gates、19/19逐门 mutations 和 182 项联合回归通过。abstract 仅开放 3 条 restricted claim，15 条 negative/blocked/prohibited claim 强制保留；T6.15.5 仍为 V5 NO-GO，T6.9.2 的 42 个 measured 字段仍全 null。下一步仅按矩阵冻结可支持的主图 1--2，不绘制已 Dropped 的 V5 IMM/BOCPD/posterior-mixture 结果）。
...
| T7.1.2 | In Progress | 冻结主图 1—2。 | Fig.1：T7.1.1 证据边界、已实现的 unified contract 与 host/FPGA 双回路；Fig.2：仅绘制 V4 已实现的 observed features、regime-aware safe policy、trusted action、A/B/LKG/rollback/hysteresis 与 6-cycle fast path。T6.10.1 已早停，IMM/BOCPD、posterior-mixture MAP、V5 risk compiler 不得作为结果或已实现模块进入主图；若保留概念路线，只能放入 Discussion/Supplement 并标 `not run/dropped`。 | `T6.7--T6.9`, `T6.10.1`, `T6.15.5`, `T7.1.1` |
| T7.1.3 | Todo | 冻结主图 3—4。 | Fig.3：V4 untouched smooth + abrupt/OOD 的 strongest-baseline LER、tail safety、oracle gap、lag/fallback 与明确负结果；Phase 6C 正结果仅按独立 secondary lane 放入 Supplement。Fig.4：现有 fixed-point/CXXRTL、6-cycle/II=1 与多 seed P&R estimate；已 Dropped 的 V5 quantized/formal/P&R 不得补图。T6.9.2 未过时，measured latency/deadline/power panel保持明确空缺。 | `T6.7--T6.9`, `T6.15.5`, `T6.19.3`, `T7.1.1` |
