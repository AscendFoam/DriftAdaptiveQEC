...
当前推荐任务：`T6.25.2`（T6.25.1 已确认旧 artifacts 与带 direct binding 的源码均 live，同时保留 T6.2.1/2 缺 input-source binding 的限制；当前没有一个 actual top 同时包含 production CRC32/staging/CAS/drain、Route-A policy/LKG 与 target synthesis surface：T6.2.2 是 raw-pin core 百万周期，T6.7.3 是 policy+raw-pin core，T6.9.1 P&R 又包裹后者。15/15 gates、20/20 mutations、13 focused tests 通过，旧 PASS 只作 regression/reference。下一步必须先构建唯一 converged production top，再对同一 top 做 property/cover/mutation、百万周期 CXXRTL 和三种子 P&R；multimode calibration/pilot/formal 保持未访问，multimode decoder、板测与 fastest claim 保持关闭）。
...
| T6.25.2 | In Progress | 构建唯一 converged production top，并对该 actual parameterized RTL 完成 atomic/fail-closed property、cover 与 mutation closure。 | 同一 top 必须同时实例化 production CRC32/staging/CAS/drain ownership、Route-A policy/LKG 和 single-mode 6-cycle core，且不暴露 raw trust/config bypass。证明 A/B commit 只能 old-or-new、读写隔离、CRC/version/age/ack、LKG rollback、commit/cancel/drain/reset/deadline、FIFO/backpressure 和 near-wrap；任何 violation 必须进入 fail-closed。对每个 property 提供 reachable cover 与 targeted mutation kill；sampled simulation 不能代替全状态证明。 | `T6.25.1`, `R-N132`, `R-N155`, `T6.15.3` |
