...
当前推荐任务：`T6.26.3`（T6.25.4 已对 T6.25.3 exact qualified converged source 完成 GW2AR seeds 1/7/19 synthesis/P&R：Fmax=`37.869/37.736/36.794` MHz，全部通过 27 MHz；最大资源 LUT4=6130、DFF=2495、BSRAM=8、DSP primitives=2。16/16 gates、19/19 semantic mutations、14 focused/83 joint tests 通过，Yosys/nextpnr/source/netlist/raw reports 均 hash-bound。三条 critical path 均从 production core 终止于 observability fold，故只允许写 whole-harness pre-board conservative estimate；multimode pilot/formal 保持未访问，板测、fastest、multimode decoder-in-RTL 继续关闭。下一步把 multimode v1 NO-GO/null、single-mode RTL GO 证据和 learning absent/dropped 明确分 panel、分 claim、分 Source Data，不做跨 lane 补门）。
...
| T6.26.3 | In Progress | 生成不混淆的双 lane claim-evidence matrix、Source Data 和主图合同。 | multimode 图只呈现 LER/tail/scaling/compute；single-mode 图只呈现 6-cycle/II=1、atomic/fail-closed、CXXRTL/formal/P&R 与 board-null；learning 以 inset/ablation 标注。每个元素绑定 raw/config/code/source/hash 和 allowed/forbidden wording，mutation 阻断跨 lane 箭头与补门。 | `T6.24.5`, `T6.25.4`, `T6.26.2`, `R-N147`, `R-N153` |
