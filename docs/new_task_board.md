...
当前推荐任务：`T7.1.4`（T7.1.3 已冻结主图 3--4：55-row evidence-bound Source Data、16/16 gates/mutations、8 个多格式输出和 153 项父链联合回归通过。Fig.3 显式保留 Window strongest、static/Route-A 与 oracle-gap closure 负结果、六类 abrupt/OOD 仅 non-inferiority 及高 fallback/recovery 成本；Phase 6C task-local 结果只进 Supplement。Fig.4 只开放 1,000,000-cycle CXXRTL 0 mismatch/undefined/overflow、6-cycle/II=1 clock model 与 2 profiles×3 seeds P&R estimate；42 board fields 仍 null、V5 hardware 仍 Dropped。下一步冻结 Supplement figure contract）。
...
| T7.1.4 | In Progress | 冻结 Supplement figure contract。 | 将 cutoff/gradient、noise-transfer 有效域、Petz bound、top-K Pareto、六 Pauli states、all-seed distributions、完整 OOD、fixed-point、failure modes，以及 Phase 6C 六条独立 comparison lane 放入 Supplement；禁止把 atlas 压成全局排名，主文保持单一 V5 论证主线。 | `T6.19.3`, `experiment_plan.md §15.5, §16.2` |
