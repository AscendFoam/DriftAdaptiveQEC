...
当前推荐任务：`T7.1.3`（T7.1.2 已冻结主图 1--2：38 个图元素逐项绑定 live report/Source Data/code，16/16 fail-closed gates 与逐门 mutations 通过；Python-only 183 mm 双栏图已导出 editable SVG/PDF、300-dpi PNG 和 600-dpi LZW-TIFF，并完成人工视觉 QA。Fig.1 只声明 6-cycle/II=1 的 CXXRTL 预板合同与三 seed P&R estimate，42 个板测字段仍为 null；Fig.2 将 IMM/BOCPD/posterior-mixture/V5 compiler/quantized/formal/P&R 明示为 `NOT RUN / DROPPED`，不接入生产箭头。下一步按同一证据边界冻结主图 3--4）。
...
| T7.1.3 | In Progress | 冻结主图 3—4。 | Fig.3：V4 untouched smooth + abrupt/OOD 的 strongest-baseline LER、tail safety、oracle gap、lag/fallback 与明确负结果；Phase 6C 正结果仅按独立 secondary lane 放入 Supplement。Fig.4：现有 fixed-point/CXXRTL、6-cycle/II=1 与多 seed P&R estimate；已 Dropped 的 V5 quantized/formal/P&R 不得补图。T6.9.2 未过时，measured latency/deadline/power panel保持明确空缺。 | `T6.7--T6.9`, `T6.15.5`, `T6.19.3`, `T7.1.1` |
