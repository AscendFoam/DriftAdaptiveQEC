...
当前推荐任务：`T6.25.1`（T6.20.4 已在全 13-family、12 个 d=3 train seed-cluster、79,872 轮上得到 direct `NO_GO_MULTIMODE_CAUSAL_HEADROOM`：最强当前可执行 baseline 是 static-mixture exact MLD，`p_L=0.111979`；trusted-bank risk action 把未保护 PP 的明显退化拉回同一 `p_L`，paired relative headroom=`0% [0%,0%]`，远低于 `15%/12%` 门。BSV/T-join/alias/causality、indirect-privilege 与 15 类 mutation 完整性检查全过；Phase 6D v1 的 T6.21--T6.24 与 T6.26.1--T6.26.2 已按预注册 early-stop Dropped，calibration/pilot/formal 保持未访问。下一步只推进独立的 single-mode RTL lane）。
