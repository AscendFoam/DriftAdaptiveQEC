...
当前推荐任务：`T6.25.3`（T6.25.2 已构建唯一 `gkp_route_a_converged_production_top`，同一 top 内含 CRC32/full-image staging、独立 image/activation version、CAS/cancel/drain/snapshot、Route-A policy/LKG 与真实六周期 core，外部无 raw cfg/trust bypass。形式检查先发现并修复 commit ACK 返回前重复呈现一周期与 host-CAS 不可达两项缺陷；mutation 又发现两步 actual-core atomic harness 的初始化空窗，增至三步后正确 core 通过且错误版本 core 被反例杀死。最终 compositional unbounded safety、20-cycle BMC、真实 core 原子提交细化、14/14 reachable witnesses、21/21 independent formal mutation kills、17/17 gates、13 focused tests 均通过。下一步只对该 exact top 重跑 aggregate `>=1e6` independent-golden/CXXRTL；旧 raw-pin 百万周期不转移，multimode calibration/pilot/formal 保持未访问，multimode decoder、板测与 fastest claim 保持关闭）。
...
| T6.25.3 | In Progress | 重跑或实时验证 long-sequence independent-golden/CXXRTL qualification。 | 仅以 T6.25.2 exact converged top 与 live source hashes 为对象；每 family `>=1e5` cycles、aggregate `>=1e6`，source-to-action 恰 6 cycles、II=1 无 bubble、零 bit mismatch、零 undefined action、零 silent overflow、零 silent version wrap；fault recovery 与 reason code 逐事件对拍。旧 T5.5.1 binding 已有漂移，必须重建 current-source independent golden，不能复制 legacy PASS。只报 pre-board，不填 measured deadline/jitter。 | `T6.2.2`, `T6.7.3`, `R-N132`, `R-N156` |
