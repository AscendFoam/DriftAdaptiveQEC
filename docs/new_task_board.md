...
当前推荐任务：`T7.2.5`（T7.2.4 已完成 limitations/cost/physical-transition-bounded Discussion/Conclusion：45 页主 note 新增 8 个有序 Discussion 小节、项目成本表和 real-data intake→shadow→board/HIL→guarded QPU→physical effectiveness→break-even 六级单调证据阶梯。27-row 六态合同以 22/22 gates/mutations 冻结 static/Window 反例、59--96% 干预成本、V5 causal/action early-stop、Phase 6C 非营救、single-mode/effective 外部效度、无 cavity/transmon、无真实 lifetime/beyond-break-even、无板上训练、无 measured speed/power 与 42 board-null；10 focused/70 adjacent tests、45 页日志和全页视觉 QA 通过。下一步完成公式、参数、完整 baseline/CI/消融/失败/RTL/工具链/长序列和 Phase 6C source locator 的 Supplementary）。
...
| T7.2.5 | In Progress | 完成 Supplementary。 | 包含公式、参数表、完整 baseline、置信区间、消融、失败模式、RTL/工具链、长序列和复现说明；附 Phase 6C source locator、task signature、N/A/null/failed 状态、外部复现偏差和非混排比较 atlas。 | `T6.16--T6.19`, `experiment_plan.md:1070` |
